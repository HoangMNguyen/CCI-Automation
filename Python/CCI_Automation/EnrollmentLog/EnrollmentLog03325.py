...
def EnrollmentLog03325(final_data):
    if not final_data["DM"].empty:
        # DM
        DM_df = final_data['DM'][['Subject','Race (IG_NS_NA_DM1.CL_NS_YH_RACE_cl_NS_DMRACE1)', 'Ethnicity (IG_NS_NA_DM1.CL_NS_NH_ETHNIC_cl_NS_DMETHNIC1)', 'Sex Assigned at Birth (IG_NS_NA_DM1.CL_NS_NH_BRTHSEX_cl_NS_DMSEX3)', 'Legal Sex (IG_NS_NA_DM1.CL_NS_YH_SEX_cl_NS_DMSEX1)', 'Date of Birth (IG_NS_NA_DM1.DT_NS_NH_BRTHDAT)', 'Main Consent Date (IG_NS_NA_DM1.DT_NS_YH_RFICDAT)' ]].copy()
        DM_new_col_name = {'Race (IG_NS_NA_DM1.CL_NS_YH_RACE_cl_NS_DMRACE1)': 'Race', 'Ethnicity (IG_NS_NA_DM1.CL_NS_NH_ETHNIC_cl_NS_DMETHNIC1)': 'Ethnicity', 'Sex Assigned at Birth (IG_NS_NA_DM1.CL_NS_NH_BRTHSEX_cl_NS_DMSEX3)': 'Sex Assigned at Birth' , 'Legal Sex (IG_NS_NA_DM1.CL_NS_YH_SEX_cl_NS_DMSEX1)': 'Legal Sex', 'Date of Birth (IG_NS_NA_DM1.DT_NS_NH_BRTHDAT)': 'Date of Birth', 'Main Consent Date (IG_NS_NA_DM1.DT_NS_YH_RFICDAT)' : 'Main Consent Date'}
        ...
        # DSCA
        DSCA_df = final_data['DSCA'][['Subject','Cohort Assignment (IG_NS_NA_DSCA1.CL_NS_YH_CACHASCOD_cl_NS_COHORT1)' ]].copy()
        ...
        IE_df = final_data['IE'][['Subject', 'Event Group Label', 'Date of Eligibility Confirmation by Physician-Investigator (IG_NS_NA_IE5.DT_NS_YH_ELIGPIDAT)']].copy()
        IE_df = IE_df[IE_df['Event Group Label'] == 'Step #1 Screening/Eligibility']
        IE_new_col_name = {'Date of Eligibility Confirmation by Physician-Investigator (IG_NS_NA_IE5.DT_NS_YH_ELIGPIDAT)': 'Date of Confirmation of Step #1 Eligibility by PI'}
        ...
        merged_df['Date of Confirmation of Step #1 Eligibility by PI'] = pd.to_datetime(merged_df['Date of Confirmation of Step #1 Eligibility by PI']).dt.strftime('%m/%d/%Y')
        ...
        merged_df.loc[mask, 'Age at Consent'] = merged_df[mask].apply(lambda x: relativedelta(x['Main Consent Date'], x['Date of Birth']).years, axis=1)
        ...
        IE_new_col_name = {'Date of Eligibility Confirmation by Physician-Investigator (IG_NS_NA_IE5.DT_NS_YH_ELIGPIDAT)': 'Date of Confirmation of Step #2 Eligibility by PI'}
        IE_df = IE_df.rename(columns=IE_new_col_name)
        IE_df = IE_df.drop('Event Group Label', axis = 1)
        merged_df = pd.merge(merged_df, IE_df, on='Subject', how='left')
        merged_df['Date of Confirmation of Step #2 Eligibility by PI'] = pd.to_datetime(merged_df['Date of Confirmation of Step #2 Eligibility by PI']).dt.strftime('%m/%d/%Y')
        # print(merged_df)

    #IE Date of Confirmation of Step #2 Eligibility by Monitoring
        IE_df = final_data['IE'][['Subject', 'Event Group Label', 'Date of Completion of Monitoring Visit for Eligibility (IG_NS_NA_IE5.DT_NS_YH_ELIGMONDAT)']].copy()
        IE_df = IE_df[IE_df['Event Group Label'] == 'Step #2 Screening/Eligibility']
        IE_new_col_name = {'Date of Completion of Monitoring Visit for Eligibility (IG_NS_NA_IE5.DT_NS_YH_ELIGMONDAT)': 'Date of Confirmation of Step #2 Eligibility by Monitoring'}
        IE_df = IE_df.rename(columns=IE_new_col_name)
        IE_df = IE_df.drop('Event Group Label', axis = 1)
        merged_df = pd.merge(merged_df, IE_df, on='Subject', how='left')
        merged_df['Date of Confirmation of Step #2 Eligibility by Monitoring'] = pd.to_datetime(merged_df['Date of Confirmation of Step #2 Eligibility by Monitoring']).dt.strftime('%m/%d/%Y')
        # print(merged_df)

        #PRAPH
        APH_df = final_data['PRAPH'][['Subject','Apheresis Type (IG_NS_NA_PRAPH1.CL_NS_YH_APHTP_cl_NS_APHTP1)', 'Apheresis Date (IG_NS_NA_PRAPH1.DT_NS_NH_APHDAT)']].copy()
        APH_new_col_name = {'Apheresis Type (IG_NS_NA_PRAPH1.CL_NS_YH_APHTP_cl_NS_APHTP1)': 'Apheresis Type (Fresh or Historical)', 'Apheresis Date (IG_NS_NA_PRAPH1.DT_NS_NH_APHDAT)': 'Date of Apheresis Collection'}
        ...
        EXINF_new_col_name = {'Study Treatment Date (IG_NS_NA_EXINF1.DT_NS_NH_INFDAT)': 'CAR T cell Administration Date (Day 0)'}
        ...
        merged_df['CAR T cell Administration Date (Day 0)'] = pd.to_datetime(merged_df['CAR T cell Administration Date (Day 0)']).dt.strftime('%m/%d/%Y')
        # print(merged_df)
        ...
        INITLF_df = INITLF_df[INITLF_df['From which Phase is the Subject entering Long-Term Follow-Up? (IG_NS_NA_DSINITLF1.CL_NS_NH_PHASELTFU_cl_NS_PHASE1)'] != 'Retreatment']
        INITLF_new_col_name = {
        ...
        INITLF_df = INITLF_df.drop('From which Phase is the Subject entering Long-Term Follow-Up? (IG_NS_NA_DSINITLF1.CL_NS_NH_PHASELTFU_cl_NS_PHASE1)', axis = 1)
        ...
        DSINITRT_df['Initiation of Retx Date'] = DSINITRT_df['Initiation of Retx Date'].fillna(DSINITRT_df['End of Long-Term Follow-Up Date (IG_NS_NA_DSINITRT1.DT_NS_YH_LTFUENDDAT)'])
        ...
        merged_df['Last Study Visit Completed in Primary Follow-Up'] = merged_df['Last Study Visit Completed in Primary Follow-Up'].fillna(merged_df['Last Visit Completed in Primary Follow-Up (IG_NS_NA_DSINITRT1.CL_NS_NH_RELVCPFU_cl_YS_LVCPFU1)'])
        merged_df = merged_df.drop(['Last Visit Completed in Primary Follow-Up (IG_NS_NA_DSINITRT1.CL_NS_NH_RELVCPFU_cl_YS_LVCPFU1)', 'Phase'], axis = 1)
        ...
        EXINF_new_col_name = {'Study Treatment Date (IG_NS_NA_EXINF1.DT_NS_NH_INFDAT)': 'CAR T Cell Retreatment Date (Day 0-R)'}
        EXINF_df = EXINF_df.rename(columns=EXINF_new_col_name)
        EXINF_df = EXINF_df.drop('Event Group Label', axis = 1)
        merged_df = pd.merge(merged_df, EXINF_df, on='Subject', how='left')
        ...
        #INITLF Retx
        INITLF_df = final_data['DSINITLF'][['Subject','From which Phase is the Subject entering Long-Term Follow-Up? (IG_NS_NA_DSINITLF1.CL_NS_NH_PHASELTFU_cl_NS_PHASE1)', 'Last Study Visit Completed in Retreatment (IG_NS_NA_DSINITLF1.CL_NS_NH_LVCRETX_cl_NS_LVCPFUR1)', 'End of Retreatment Date (IG_NS_NA_DSINITLF1.DT_NS_YH_RETXENDDAT)']].copy()
        INITLF_df = INITLF_df[INITLF_df['From which Phase is the Subject entering Long-Term Follow-Up? (IG_NS_NA_DSINITLF1.CL_NS_NH_PHASELTFU_cl_NS_PHASE1)'] == 'Retreatment']
        INITLF_new_col_name = {'Last Study Visit Completed in Retreatment (IG_NS_NA_DSINITLF1.CL_NS_NH_LVCRETX_cl_NS_LVCPFUR1)': 'Last Study Visit Completed in Retreatment (X-R)', 'End of Retreatment Date (IG_NS_NA_DSINITLF1.DT_NS_YH_RETXENDDAT)': 'Initiation of Retreatment LTFU Date'}
        INITLF_df = INITLF_df.rename(columns=INITLF_new_col_name)
        INITLF_df = INITLF_df.drop('From which Phase is the Subject entering Long-Term Follow-Up? (IG_NS_NA_DSINITLF1.CL_NS_NH_PHASELTFU_cl_NS_PHASE1)', axis = 1)
        merged_df = pd.merge(merged_df, INITLF_df, on='Subject', how='left')
