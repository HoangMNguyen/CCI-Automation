...
def EnrollmentLog50425(final_data):
    ...
    if "MHDIAGA" in final_data and not final_data["MHDIAGA"].empty:
        ...
        MHDIAGA_df = final_data["MHDIAGA"][list(MHDIAGA_cols_to_select.keys())].rename(columns=MHDIAGA_cols_to_select)
    ...
    if "MHDIAGB" in final_data and not final_data["MHDIAGB"].empty:
        ...
        MHDIAGB_df = final_data["MHDIAGB"][list(MHDIAGB_cols_to_select.keys())].rename(columns=MHDIAGB_cols_to_select)
    ...
    if ("MHDIAGA" in final_data and not final_data["MHDIAGA"].empty) or (
        ...
        merged_df["Disease Type at Study Enrollment"] = merged_df["Disease Type at Study Enrollment Cohort A"].fillna(
        ...
        if "Disease Type at Study Enrollment" in merged_df.columns:
            index_reference = merged_df.columns.get_loc("Race")
            merged_df.insert(
                index_reference, "Disease Type at Study Enrollment", merged_df.pop("Disease Type at Study Enrollment")
            )
    ...
    if "EXINF" in final_data and not final_data["EXINF"].empty:
        ...
        merged_df["CART-45 Infusion Date"] = pd.to_datetime(merged_df["CART-45 Infusion Date"]).dt.strftime("%m/%d/%Y")
    ...
    if "DSEOS" in final_data and not final_data["DSEOS"].empty:
        ...
        # Convert End of Study Date to datetime
        DSEOS_df["End of Study Date"] = pd.to_datetime(DSEOS_df["End of Study Date"], errors="coerce")
