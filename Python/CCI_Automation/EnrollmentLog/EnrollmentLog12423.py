...
def EnrollmentLog12423(final_data):
    # DM
    if "DM" in final_data:
        # print("DM data exists")
        DM_df = final_data["DM"][
            [
                "Subject",
    ...
    if "PRAPH" in final_data:
        ...
        PRAPH_df = PRAPH_df[PRAPH_df["Event Group Label"] == "Initial Study Enrollment/Apheresis"].drop(
    ...
    if "EXINF" in final_data:
        EXINF_df = final_data["EXINF"][
            ["Subject", "Event Group Label", "Infusion Date (IG_NS_NA_EXINF1.DT_NS_NH_INFDAT)"]
        ].copy()
