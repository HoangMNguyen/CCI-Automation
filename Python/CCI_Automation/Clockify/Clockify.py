...
class ClockifyDashboard():
    def __init__(self, project_name, output_dir, input_file_path = None):   #self.selected_option2, self.input_folder_name, self.output_folder_name, self.output_file_name 
        ...
        if input_file_path != None:
            ...
            self.raw_data = clockify_get_detailed_report(clockify_get_api_key(), clockify_get_workplace_id(), project_name)
    ...
    def collect_data(self):
        ...
        total_instance = {'Task': 'Total Project Hours', 'Total Task Hours': df1_total_hours, 'Total Hours Percentage per Task': ''}
        ...
        # Replace NaN values with 0
        self.df2['Project Milestone Percentage of Project'] = self.df2['Project Milestone Percentage of Project'].replace(np.nan, 0)
    ...
    def output(self):
        self.output_file_name = date.today().strftime("%y%m%d") + "-" + self.filter_data.iloc[0,0] + "-Clockify Dashboard"
        with pd.ExcelWriter(self.output_dir  + '/' + self.output_file_name + '.xlsx', engine = 'xlsxwriter') as writer:  
            ...
            self.df1.to_excel(writer, sheet_name=self.project_name + ' Dashboard', index = False, startcol=1, startrow=3)
            self.df2.to_excel(writer, sheet_name=self.project_name + ' Dashboard', index = False, startcol=5, startrow=3)
            self.df3.to_excel(writer, sheet_name=self.project_name + ' Dashboard', index = False, startcol=5, startrow=11)
            ...
            worksheet2.autofit()
            worksheet3 = writer.sheets[self.project_name + ' Dashboard']
            worksheet3.autofit()
            ...
            ## formating
            #coloring
            no_border_white = writer.book.add_format({'bg_color': '#FFFFFF',
                                                        'border': 0})
            blue_header_format = writer.book.add_format({'bg_color': '#DCE6F1',
                                                        'text_wrap': True,
                                                        'valign': 'vcenter',
                                                        'align': 'center',
                                                        'bold' : True,
                                                        ...
                                                        'font_size' : 11,
                                                        'border': 1})
            pink_header_format = writer.book.add_format({'bg_color': '#F2DCDB',
                                                        'text_wrap': True,
                                                        'valign': 'vcenter',
                                                        'align': 'center',
                                                        'bold' : True,
                                                        ...
                                                        'font_size' : 18,
                                                        'border': 1})
            pink_data_format = writer.book.add_format({'bg_color': '#F2DCDB',
                                                        'text_wrap': True,
                                                        'valign': 'vcenter',
                                                        'align': 'right',
                                                        'bold' : True,
                                                        ...
                                                        'font_size' : 11,
                                                        'border': 1})
            grey_left_format = writer.book.add_format({'bg_color': '#F2F2F2',
                                                        'text_wrap': True,
                                                        'valign': 'vcenter',
                                                        'align': 'left',
                                                        'font_name' : 'Calibri',
                                                        'font_size' : 11,
                                                        'border': 1})
            grey_perc_left_format = writer.book.add_format({'bg_color': '#F2F2F2',
                                                        'text_wrap': True,
                                                        'valign': 'vcenter',
                                                        'align': 'left',
                                                        'font_name' : 'Calibri',
            ...
            worksheet3.merge_range('B2:H2', self.filter_data.iloc[0,0] + " Clockify Dashboard\nDate generated: " + date.today().strftime("%m/%d/%Y") , pink_header_format)
