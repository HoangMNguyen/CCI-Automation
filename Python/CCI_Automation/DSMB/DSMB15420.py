...
def DSMB15420(data, export = False, output_dir = "C:/Users/Hoang Nguyen/Dropbox/Current Work/Download", output_file_name = datetime.now().strftime("%Y%m%d%H%M%S") + "-15420-DSMB Report", debug = False):
    #if not data['DM'].empty:
    #Subject
    enrollment_df = data['DM'][['Subject']].copy()
    enrollment_df = enrollment_df.sort_values(['Subject'])
    #Cohort Assignment
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DSCA', 'Cohort Assignment (IG_NS_NA_DSCA1.CL_NS_NH_CACHASCOD_cl_NS_COHORT1)', 'Cohort Assignment')
    #Disease
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAG', 'Primary Diagnosis of CLL (ig_PRDIAG2.PRDIAGCLL)', 'Disease CLL')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAG', 'Specify Other Diagnosis (ig_PRDIAG2.PRDIAGCLLOTH)', 'Disease CLL2')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAG', 'Primary Diagnosis of NHL (ig_PRDIAG3.PRDIAGNHL)', 'Disease NHL')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAG', 'Specify Other Diagnosis (ig_PRDIAG3.PRDIAGNHLOTH)', 'Disease NHL2')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAGALL', 'Primary Diagnosis of ALL (ig_PRDIAGALL1.PRDIAGALL)', 'Disease ALL')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAGALL', 'Specify Other Diagnosis (ig_PRDIAGALL1.PRDIAGALLOTH)', 'Disease ALL2')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'PRDIAG', 'Primary Diagnosis at Enrollment (ig_PRDIAG5.PRDIAGRICH)', 'Disease RICH')
    enrollment_df['Disease'] = None
    # List the columns in the order you want to use them for filling 'Disease'
    columns_to_fill_from = ['Disease CLL2', 'Disease CLL', 'Disease NHL2', 'Disease NHL', 'Disease ALL2', 'Disease ALL', 'Disease RICH' ]
    ...
    enrollment_df = enrollment_df.drop(columns = ['Disease CLL', 'Disease CLL2', 'Disease NHL2', 'Disease NHL', 'Disease ALL2', 'Disease ALL', 'Disease RICH'])
    #Dose Level
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DLA', 'Dose Level Assignment (ig_DLA1.DLADOSELVL)', 'Dose Level')
    #Legal Sex
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DM', 'Legal Sex (ig_DM1.SEX)', 'Legal Sex')
    #Sex Assigned at Birth 
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DM', 'Sex Assigned at Birth (ig_DM1.BRTHSEX)', 'Sex Assigned at Birth')
    #Gender Identity
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DM', 'Gender Identity (ig_DM1.GENDERID2)', 'Gender Identity')
    #Age
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'DM', 'Date of Birth (ig_DM1.BRTHDAT)', 'Date of Birth')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Consent Date (ig_IE1.MAINCDAT)', 'Consent Date')
    ...
    enrollment_df.loc[mask, 'Age'] = enrollment_df[mask].apply(lambda x: relativedelta(x['Consent Date'], x['Date of Birth']).years, axis=1)
    ...
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Subject meets all study eligibility (ig_IE3.IEYN)', 'Subject meets all study eligibility?')
    #Reason for Screen Failure
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Other Screen Fail Reason (ig_IE4.OTHRSFREAS)', 'Reason for Screen Failure')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Screen Failure Reason (ig_IE4.IECAT)', 'SF1')
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Select the primary inclusion criterion excluding this subject  (ig_IE4.ITESTCD)', 'SF2')
    enrollment_df['SF2'] = enrollment_df[enrollment_df['SF2'].notna()]['SF2'].astype(int)
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'IE', 'Select the primary exclusion criterion excluding this subject (ig_IE4.ETESTCD)', 'SF3')
    enrollment_df['SF3'] = enrollment_df[enrollment_df['SF3'].notna()]['SF3'].astype(int)
    enrollment_df['SF4'] = enrollment_df['SF1'].fillna('').astype(str) + " " + enrollment_df['SF2'].round().fillna('').astype(str) + enrollment_df['SF3'].round().fillna('').astype(str)
    ...
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'INF', 'Was Infusion Administered? (ig_INF1.INFOCCUR)', 'Infused')
    ...
    enrollment_df = add_rename_column_corelisting(enrollment_df, data, 'EOS', 'End of Study Date (ig_EOS1.EOSDAT)', 'End of Study Date (ig_EOS1.EOSDAT)')
    # Update 'Infused' column based on the conditions: 
    enrollment_df.loc[(enrollment_df['Infused'] != 'Yes') & (enrollment_df['End of Study Date (ig_EOS1.EOSDAT)'].isnull()), 'Infused'] = 'Pending'
    enrollment_df.loc[(enrollment_df['Infused'] != 'Yes') & (~enrollment_df['End of Study Date (ig_EOS1.EOSDAT)'].isnull()), 'Infused'] = 'No'
    ...
    CAE = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort A')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')].count()['Subject']
    CAI = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort A')) & (enrollment_df['Infused'] == 'Yes')].count()['Subject']
    CAE_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort A')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')]
    CAI_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort A')) & (enrollment_df['Infused'] == 'Yes')]
    #Cohort B Enrolled & Infused
    CBE = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort B')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')].count()['Subject']
    CBI = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort B')) & (enrollment_df['Infused'] == 'Yes')].count()['Subject']
    CBE_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort B')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')]
    CBI_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort B')) & (enrollment_df['Infused'] == 'Yes')]
    #Cohort C Enrolled & Infused
    CCE = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort C')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')].count()['Subject']
    CCI = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort C')) & (enrollment_df['Infused'] == 'Yes')].count()['Subject']
    CCE_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort C')) & (enrollment_df['Subject meets all study eligibility?'] == 'Yes')]
    CCI_df = enrollment_df.fillna('').loc[(enrollment_df['Cohort Assignment'].fillna('').str.contains('Cohort C')) & (enrollment_df['Infused'] == 'Yes')]
    ...
    final_status = {'Total Screened' : TT, 'Screen Failed' : SF, 'Cohort A Enrolled' : CAE, 
                'Cohort A Infused' : CAI, 'Cohort B Enrolled' : CBE, 'Cohort B Infused' : CBI, 
                'Cohort C Enrolled' : CCE, 'Cohort C Infused' : CCI}
    ...
    TCD_dict = {'Dose Level 1a (DL1a)' : 3000000,
    ...
    grouped_df.loc[(grouped_df['Event Group Label'] == 'Lymphodepleting Chemotherapy') | (grouped_df['Event Group Label'] == 'Lymphodepleting Chemotherapy - ALL'), 'Event Group Label'] = 'Day 0'
    grouped_df.loc[(grouped_df['Event Group Label'] == 'Retreatment Lymphodepleting Chemotherapy') | (grouped_df['Event Group Label'] == 'Retreatment Lymphodepleting Chemotherapy - ALL'), 'Event Group Label'] = 'Day 0-R'
    ...
    #Cohort Assignment
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'DSCA', 'Cohort Assignment (IG_NS_NA_DSCA1.CL_NS_NH_CACHASCOD_cl_NS_COHORT1)', 'Cohort Assignment')
    #Dose Level
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'DLA', 'Dose Level Assignment (ig_DLA1.DLADOSELVL)', 'Dose Level Assignment')
    #Lymphodepleting Chemotherapy Regimen
    infusion_df = add_rename_column_df(infusion_df, EXCHMO_df[EXCHMO_df['Event Group Label'] == 'Day 0'], 'EXCHMO', 'Medication (ig_EXCHMO2.EXCCAT)', 'Lymphodepleting Chemotherapy Regimen')
    # Fill NaN with 
    #Infusion Date
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'Infusion Date (ig_INF1.INFDAT)', 'Date of huCART19-IL18 Infusion', 'Subject', 'Event Group Label')
    # convert the date to datetime object and format it to MM-DD-YYYY
    infusion_df['Date of huCART19-IL18 Infusion'] = infusion_df['Date of huCART19-IL18 Infusion'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d').strftime('%m-%d-%Y') if pd.notna(x) else x)
    ...
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'CAR T Cell Dose Administered (ig_INF1.INFDOS)', 'Total huCAR T Cell Dose Administered', 'Subject', 'Event Group Label')
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'x 10 to the power of (ig_INF1.INFDOSXP)', 'x 10 to the power of (ig_INF1.INFDOSXP)', 'Subject', 'Event Group Label')
    ...
    infusion_df['Total huCAR T Cell Dose Administered'] = infusion_df['Total huCAR T Cell Dose Administered'].multiply(10**infusion_df['x 10 to the power of (ig_INF1.INFDOSXP)'])
    infusion_df = infusion_df.drop(columns = ['x 10 to the power of (ig_INF1.INFDOSXP)'])
    
    #Total Cell Dose Administered column
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'Total Cell Dose Administered (ig_INF1.INFDOSTOT)', 'Total Cell Dose Administered', 'Subject', 'Event Group Label')
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'x 10 to the power of (ig_INF1.INFDOSTOTXP)', 'x 10 to the power of (ig_INF1.INFDOSTOTXP)', 'Subject', 'Event Group Label')
    infusion_df['Total Cell Dose Administered'] = infusion_df['Total Cell Dose Administered'].multiply(10**infusion_df['x 10 to the power of (ig_INF1.INFDOSTOTXP)'])                
    ...
    infusion_df['Met Target Dose'] = infusion_df.apply(lambda row: "Y" if isinstance(row['Target Cell Dose'], int) and row['Total huCAR T Cell Dose Administered'] >= row['Target Cell Dose'] else "", axis=1)
    infusion_df['Met Target Dose'] = infusion_df.apply(lambda row: "N" if isinstance(row['Target Cell Dose'], int) and row['Total huCAR T Cell Dose Administered'] < row['Target Cell Dose'] else row['Met Target Dose'], axis=1)

    #%scFv Flow
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'Transduction Efficiency (%) (ig_INF1.INFTEFFP)', '%scFv Flow', 'Subject', 'Event Group Label')

    #adding Met Target %scFv
    infusion_df = add_rename_column_corelisting(infusion_df, data, 'INF', 'Transduction Efficiency (%) (ig_INF1.INFTEFFP)', 'Met Target %scFv', 'Subject', 'Event Group Label')
    ...
    #Cohort Assignment
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'DSCA', 'Cohort Assignment (IG_NS_NA_DSCA1.CL_NS_NH_CACHASCOD_cl_NS_COHORT1)', 'Cohort Assignment')
    #Lymphodepleting Chemotherapy Regimen
    infusionR_df = add_rename_column_df(infusionR_df, EXCHMO_df[EXCHMO_df['Event Group Label'] == 'Day 0-R'], 'EXCHMO', 'Medication (ig_EXCHMO2.EXCCAT)', 'Lymphodepleting Chemotherapy Regimen')
    #Infusion Date
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'Infusion Date (ig_INF1.INFDAT)', 'Date of huCART19-IL18 Infusion', 'Subject', 'Event Group Label')
    # convert the date to datetime object and format it to MM-DD-YYYY
    infusionR_df['Date of huCART19-IL18 Infusion'] = infusionR_df['Date of huCART19-IL18 Infusion'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d').strftime('%m-%d-%Y') if pd.notna(x) else x)
    
    #Total huCart19-IL18 Cell Dose
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'CAR T Cell Dose Administered (ig_INF1.INFDOS)', 'Total huCAR T Cell Dose Administered', 'Subject', 'Event Group Label')
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'x 10 to the power of (ig_INF1.INFDOSXP)', 'x 10 to the power of (ig_INF1.INFDOSXP)', 'Subject', 'Event Group Label')
    #combine Total huCart19-IL18 Cell Dose and x 10 to the power of (ig_INF1.INFDOSXP) columns, compare the new value with 'Target Cell Dose', and convert the Total huCart19-IL18 Cell Dose column to string
    infusionR_df['Total huCAR T Cell Dose Administered'] = infusionR_df['Total huCAR T Cell Dose Administered'].multiply(10**infusionR_df['x 10 to the power of (ig_INF1.INFDOSXP)'])
    infusionR_df = infusionR_df.drop(columns = ['x 10 to the power of (ig_INF1.INFDOSXP)'])
    
    #Total Cell Dose Administered column
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'Total Cell Dose Administered (ig_INF1.INFDOSTOT)', 'Total Cell Dose Administered', 'Subject', 'Event Group Label')
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'x 10 to the power of (ig_INF1.INFDOSTOTXP)', 'x 10 to the power of (ig_INF1.INFDOSTOTXP)', 'Subject', 'Event Group Label')
    infusionR_df['Total Cell Dose Administered'] = infusionR_df['Total Cell Dose Administered'].multiply(10**infusionR_df['x 10 to the power of (ig_INF1.INFDOSTOTXP)'])                
    infusionR_df = infusionR_df.drop(columns = ['x 10 to the power of (ig_INF1.INFDOSTOTXP)'])
    
    #%scFv Flow
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'Transduction Efficiency (%) (ig_INF1.INFTEFFP)', '%scFv Flow', 'Subject', 'Event Group Label')

    #adding Met Target %scFv
    infusionR_df = add_rename_column_corelisting(infusionR_df, data, 'INF', 'Transduction Efficiency (%) (ig_INF1.INFTEFFP)', 'Met Target %scFv', 'Subject', 'Event Group Label')
    # fillter out the rows that have NaN in Met Target %scFv
    ...
    infusion_statB2['Met Target Dose'] = str(met_target_count) + " (" + str(round(met_target_count/total_subject_count*100, 2)) + "%)"
    # Create a new dataframe for %scFv Flow table with infusion_df
    ...
    infusion_statB3['Met Target %scFv'] = str(met_target_count) + " (" + str(round(met_target_count/total_subject_count*100, 2)) + "%)"
    # Combine the three dataframes
    ...
    infusion_statC2['Met Target Dose'] = str(met_target_count) + " (" + str(round(met_target_count/total_subject_count*100, 2)) + "%)"
    # Create a new dataframe for %scFv Flow table with infusion_df
    ...
    infusion_statC3['Met Target %scFv'] = str(met_target_count) + " (" + str(round(met_target_count/total_subject_count*100, 2)) + "%)"
    # Combine the three dataframes
    ...
    infusion_df['Target Cell Dose'] = infusion_df['Target Cell Dose'].apply(lambda x: convert_float_2_sci_notation(x) if isinstance(x, int) and pd.notna(x) else x)
    infusion_df['Total huCAR T Cell Dose Administered'] = infusion_df['Total huCAR T Cell Dose Administered'].apply(lambda x: convert_float_2_sci_notation(x) if isinstance(x, float) and pd.notna(x) else x)
    infusion_df['Total Cell Dose Administered'] = infusion_df['Total Cell Dose Administered'].apply(lambda x: convert_float_2_sci_notation(x) if isinstance(x, float) and pd.notna(x) else x)
    ...
    # TODO: Day 0-R
    # Convert the columns to scientific notation if the value is not NaN
    infusionR_df['Total huCAR T Cell Dose Administered'] = infusionR_df['Total huCAR T Cell Dose Administered'].apply(lambda x: convert_float_2_sci_notation(x) if isinstance(x, float) and pd.notna(x) else x)
    ...
    DR_NHL_PET_dict = {'Complete Metabolic Response (CMR)' : 1, 'Partial Metabolic Response (PMR)' : 2, 'No Metabolic Response (NMR)' : 3, 'Indeterminate Response (IR)' : 4, 'Progressive Metabolic Disease (PMD)' : 5, 'Not Assessed' : 6, 'Not Evaluable' : 7, 'Not Reported': 10}
    # Disease Response NHL CT based dictionary
    DR_NHL_CT_dict = {'Complete Radiologic Response (CR)' : 1, 'Partial Response (PR)' : 2, 'Stable Disease (SD)' : 3, 'Indeterminate Response (IR)' : 4, 'Progressive Disease (PD)' : 5, 'Not Assessed' : 6, 'Not Evaluable' : 7, 'Not Reported': 10}
    # Disease Response CLL Overall dictionary 
    DR_CLL_OV_dict = {'Complete Remission (CR)' : 1, 'Complete Remission with Incomplete Marrow Recovery (CRi)' : 2, 'Partial Remission (PR)' : 3, 'Stable Disease (SD)' : 4, 'Progressive Disease (PD)' : 5, 'Not Assessed' : 6, 'Not Evaluable' : 7, 'Not Reported': 10}
    # Disease Response CLL Bone Marrow dictionary
    DR_CLL_BM_dict = {'Complete Remission (CR)' : 1, 'Partial Remission (PR)' : 2, 'Progressive Disease (PD)' : 3, 'Stable Disease (SD)': 4,  'Not Assessed' : 5, 'Not Evaluable' : 6, 'Not Reported': 10}
    # Disease Response ALL Overall Resposne dictionary
    DR_ALL_OV_dict = {'Complete Remission (CR)' : 1, 'Complete Remission with Incomplete Blood Count Recovery (CRi)' : 2, 'Complete Remission with Residual Mediastinal Disease (CRu)' : 3, 'Treatment Failure (TF)' : 4, 'Relapsed Disease (RD)' : 5, 'Extramedullary Disease Without Bone Marrow Involvement' : 6, 'No Clinical Evidence of Relapse' : 7, 'Unknown/Not Assessed' : 8, 'Not Reported': 9, 'Not Applicable': 10}
    # Disease Response ALL Extramedullary Disease without Bone Marrow Involvement dictionary
    DR_ALL_ED_dict = {'Complete Remission (CR)' : 1, 'Partial Remission (PR)' : 2, 'Stable Disease (SD)' : 3, 'Indeterminate Response' : 4, 'Progressive Disease (PD)' : 5, 'Not Assessed' : 6, 'Not Reported': 9, 'Not Applicable': 10}
    # Event Label Update dictionary for cohort A and B
    event_AB_dict = {'Primary Treatment and Follow-Up': 'Primary Treatment', 'Primary Retreatment and Follow-up': 'Primary Retreatment', 'Pre-Retreatment Safety Visit' : 'Pre-Retreatment', 'Long Term Follow-up Months 3-60': 'Long Term Follow-up', 'Retreatment Long Term Follow-up Months 3-60': 'Retreatment Long Term Follow-up'}
    # Event Label Update dictionary for cohort C
    event_C_dict = {'Primary Treatment and Follow-up - ALL': 'Primary Treatment', 'Primary Retreatment and Follow-up - ALL': 'Primary Retreatment', 'Pre-Retreatment Safety Visit - ALL': 'Pre-Retreatment', 'Retreatment Lymphodepleting Chemotherapy - ALL': 'Retreatment Lymphodepleting',  'Long Term Follow-up Months 3-60 - ALL': 'Long Term Follow-up', 'Retreatment Long Term Follow-up Months 3-60 - ALL': 'Retreatment Long Term Follow-up'}
    
    # Get data from Initiation of Long Term Follow up
    PD_df = data['INITLF'][['Subject', 'From which Phase is the Subject entering Long-Term Follow-Up? (ig_INITLF1.DSPHASE)', 'End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)', 'Provide reason the Subject is entering into the Long-Term Follow-Up Phase (ig_INITLF2.DSLTFURE)']].copy()
    ...
    PD_df = PD_df[PD_df['Provide reason the Subject is entering into the Long-Term Follow-Up Phase (ig_INITLF2.DSLTFURE)'] == 'Disease progression']
    # Filter the data to subject in Primary Follow up
    PD_df = PD_df[PD_df['From which Phase is the Subject entering Long-Term Follow-Up? (ig_INITLF1.DSPHASE)'] == 'Primary Follow-Up']
    # Convert End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT) to datetime object
    PD_df['End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)'] = pd.to_datetime(PD_df['End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)'])
    
    # Get data from Initiation of Long Term Follow up
    PD_Retx_df = data['INITLF'][['Subject', 'From which Phase is the Subject entering Long-Term Follow-Up? (ig_INITLF1.DSPHASE)', 'End of Retreatment Date (ig_INITLF1.DSENRETXDAT)', 'Provide reason the Subject is entering into the Long-Term Follow-Up Phase (ig_INITLF2.DSLTFURE)']].copy()
    # Filter the data to only subject with 'Disease progression' in Provide reason the Subject is entering into the Long-Term Follow-Up Phase (ig_INITLF2.DSLTFURE) column
    PD_Retx_df = PD_Retx_df[PD_Retx_df['Provide reason the Subject is entering into the Long-Term Follow-Up Phase (ig_INITLF2.DSLTFURE)'] == 'Disease progression']
    # Filter the data to subject in Retreatment
    PD_Retx_df = PD_Retx_df[PD_Retx_df['From which Phase is the Subject entering Long-Term Follow-Up? (ig_INITLF1.DSPHASE)'] == 'Retreatment']
    # Convert End of Retreatment Date (ig_INITLF1.DSENRETXDAT) to datetime object
    PD_Retx_df['End of Retreatment Date (ig_INITLF1.DSENRETXDAT)'] = pd.to_datetime(PD_Retx_df['End of Retreatment Date (ig_INITLF1.DSENRETXDAT)'])
    
    
    # Get data from DSINITRT
    DSINITRT_df = data['DSINITRT'][['Subject', 'From which Phase is the Subject entering Retreatment? (ig_DSINITRT1.DSPHASER)', 'End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT)']].copy()
    # Convert End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT) to datetime object
    DSINITRT_df['End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT)'] = pd.to_datetime(DSINITRT_df['End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT)'])
    
    
    
    # TODO: RESPONSE LISTING NHL ONLY
    # Response data dataframe for NHL only
    response_df = data['RS'][['Subject', 'Event Group Label', 'Event Date', 'Disease Type (ig_RS1.RSCAT)', 'Study Phase (ig_RS1.STUDYPHS2)', 'Primary Treatment Time Point (ig_RS1.RSTPT)', 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)',  'Retreatment Time Point (ig_RS1.RSTPTR)', 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)', 'PET-Based Response (ig_RS3.RSIMAGTYP1)','CT-Based Response (ig_RS3.RSIMAGTYP2)', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']].copy()
    ...
    responseA_primary_df = convert_integers_to_strings(responseA_primary_df, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)')
    responseA_primary_df.loc[temp_mask, 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = "Day " + responseA_primary_df.loc[temp_mask, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseA_primary_df = responseA_primary_df[responseA_primary_df['Primary Treatment Time Point (ig_RS1.RSTPT)'] != 'Pre-Treatment Safety Visit' ]
    ...
    if final_subject_A_prim_count > 0:
        ...
        responseA_primary_df['PET-Score'] = responseA_primary_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].map(DR_NHL_PET_dict)
        responseA_primary_df['CT-Score'] = responseA_primary_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].map(DR_NHL_CT_dict)
        ...
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseA_primary_current_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseA_primary_current_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        ...
        responseA_best_PET_df = responseA_best_PET_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column PET-Based NHL Disease Response to PET-Based Response
        responseA_best_PET_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best PET Time Point'}, inplace=True)
        # Merge left with the primary current response dataframe
        final_responseA_primary_df = pd.merge(responseA_primary_current_df,responseA_best_PET_df, on='Subject', how='left')
        ...
        responseA_best_CT_df = responseA_best_CT_df[['Subject', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column CT-Based NHL Disease Response to CT-Based Response
        responseA_best_CT_df.rename(columns={'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best CT Time Point'}, inplace=True)
        ...
        responseA_primary_M3_df = responseA_primary_M3_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Event Date']]
        ...
        responseA_primary_M3_df = pd.concat([responseA_primary_M3_df, responseA_df[~responseA_df['Subject'].isin(responseA_primary_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
        ...
        temp_df = temp_df[temp_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].notna() | temp_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].notna()]
        ...
        for index, row in responseA_primary_M3_df.iterrows():
            ...
            if pd.isna(row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']) and pd.isna(row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']):
                ...
                if row['Subject'] in PD_df['Subject'].values:
                    ...
                    end_date = PD_df[PD_df['Subject'] == row['Subject']]['End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)'].values[0]
                    ...
                    if not filtered_df.empty:
                        ...
                        responseA_primary_M3_df.loc[index, 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = temp_row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']
                        ...
                        responseA_primary_M3_df.loc[index, 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = temp_row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']
                # check if the subject is in Initiation of REtx before month 3
                    ...
                    end_date = DSINITRT_df[DSINITRT_df['Subject'] == row['Subject']]['End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT)'].values[0]
                    # find the response of the same subject with the latest event date that is before the Month 3 event date
                    filtered_df = temp_df[(temp_df['Subject'] == row['Subject'])  & (temp_df['Event Date'] <= end_date)]
                    # Check if the filtered DataFrame is empty
                    if not filtered_df.empty:
        ...
        responseA_primary_M3_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based ORR', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based ORR'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseA_primary_df = pd.merge(final_responseA_primary_df, responseA_primary_M3_df, on='Subject', how='left')
        ...
        responseA_primary_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseA_primary_df['SAE'] = final_responseA_primary_df['Subject'].apply(lambda x: 'Y' if x in responseA_primary_AE_df[responseA_primary_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
    ...
    responseA_retreatment_df = convert_integers_to_strings(responseA_retreatment_df, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)')
    responseA_retreatment_df.loc[temp_mask, 'Retreatment Time Point (ig_RS1.RSTPTR)'] = "Day " + responseA_retreatment_df.loc[temp_mask, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseA_retreatment_df = responseA_retreatment_df[responseA_retreatment_df['Retreatment Time Point (ig_RS1.RSTPTR)'] != 'Pre-Retreatment Safety Visit']
    # Convert Event Date to datetime object
    ...
    if final_subject_A_retx_count > 0:
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseA_retreatment_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseA_retreatment_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        # Convert PET-Based NHL Disease Response and CT-Based NHL Disease Response to numeric values
        responseA_retreatment_df['PET-Score'] = responseA_retreatment_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].map(DR_NHL_PET_dict)
        responseA_retreatment_df['CT-Score'] = responseA_retreatment_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].map(DR_NHL_CT_dict)
        ...
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseA_retreatment_current_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseA_retreatment_current_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        ...
        # Select the columns subject and PET-Based NHL Disease Response from responseA_best_PET_df
        responseA_best_PET_df = responseA_best_PET_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'Retreatment Time Point (ig_RS1.RSTPTR)']]
        # Rename the column PET-Based NHL Disease Response to PET-Based Response
        responseA_best_PET_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based Response', 'Retreatment Time Point (ig_RS1.RSTPTR)' : 'Best PET Time Point'}, inplace=True)
        ...
        # Merge left with the primary current response dataframe
        final_responseA_retreatment_df = pd.merge(responseA_retreatment_current_df,responseA_best_PET_df, on='Subject', how='left')
        ...
        # Select the columns subject and CT-Based NHL Disease Response from responseA_best_CT_df
        responseA_best_CT_df = responseA_best_CT_df[['Subject', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Retreatment Time Point (ig_RS1.RSTPTR)']]
        # Rename the column CT-Based NHL Disease Response to CT-Based Response
        responseA_best_CT_df.rename(columns={'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based Response', 'Retreatment Time Point (ig_RS1.RSTPTR)' : 'Best CT Time Point'}, inplace=True)
        ...
        responseA_retreatment_M3_df = responseA_df[responseA_df['Retreatment Time Point (ig_RS1.RSTPTR)'] == 'Month 3-R']
        # Selec the columns subject and PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) from responseA_retreatment_M3_df
        responseA_retreatment_M3_df = responseA_retreatment_M3_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Event Date']]
        ...
        responseA_retreatment_M3_df = pd.concat([responseA_retreatment_M3_df, responseA_df[~responseA_df['Subject'].isin(responseA_retreatment_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
            ...
            if pd.isna(row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']) and pd.isna(row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']):
                ...
                if row['Subject'] in PD_Retx_df['Subject'].values:
                    ...
                    if not filtered_df.empty:
                        ...
                        temp_row = filtered_df.iloc[-1]
                        # Replace the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the subject with the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the temp_row
                        responseA_retreatment_M3_df.loc[index, 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = temp_row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']
                        # Replace the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the subject with the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the temp_row
                        responseA_retreatment_M3_df.loc[index, 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = temp_row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']
        # Rename the column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) to PET-Based ORR
        responseA_retreatment_M3_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based ORR', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based ORR'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseA_retreatment_df = pd.merge(final_responseA_retreatment_df, responseA_retreatment_M3_df, on='Subject', how='left')
        # Fill NaN with "Not Reported" in column PET-Based ORR
        ...
        ## Checking AE and SAE for NHL primary
        # Getting AE and SAE dataframes
        responseA_retreatment_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseA_retreatment_df['SAE'] = final_responseA_retreatment_df['Subject'].apply(lambda x: 'Y' if x in responseA_retreatment_AE_df[responseA_retreatment_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
    ...
    response_df = data['RS'][['Subject', 'Form Status', 'Event Group Label', 'Event Date', 'Disease Type (ig_RS1.RSCAT)', 'Study Phase (ig_RS1.STUDYPHS2)', 'Primary Treatment Time Point (ig_RS1.RSTPT)', 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)',  'Retreatment Time Point (ig_RS1.RSTPTR)', 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)', 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)', 'Disease Response Date (ig_RS2.RSDAT)' , 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)' ]].copy()
    ...
    # Remove rows with Pre-Treatment Safety Visit
    responseB_primary_df = responseB_primary_df[responseB_primary_df['Primary Treatment Time Point (ig_RS1.RSTPT)'] != 'Pre-Treatment Safety Visit']
    ...
    if final_subject_B_prim_count > 0:
        ...
        responseB_primary_df = convert_integers_to_strings(responseB_primary_df, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)')
        responseB_primary_df.loc[temp_mask, 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = "Day " + responseB_primary_df.loc[temp_mask, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)'].astype(str)
        ...
        responseB_primary_df['OV-Score'] = responseB_primary_df['Overall CLL Disease Response (ig_RS2.RSCLLCAT)'].map(DR_CLL_OV_dict)
        responseB_primary_df['BM-Score'] = responseB_primary_df['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'].map(DR_CLL_BM_dict)
        ...
        # Fill NaN with "Not Reported" in column Overall CLL Disease Response (ig_RS2.RSCLLCAT)
        responseB_primary_current_df['Overall CLL Disease Response (ig_RS2.RSCLLCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)
        responseB_primary_current_df['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'].fillna('Not Reported', inplace=True)
        ...
        responseB_best_OV_df = responseB_best_OV_df[['Subject', 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column OV-Based CLL Disease Response to OV-Based Response
        responseB_best_OV_df.rename(columns={'Overall CLL Disease Response (ig_RS2.RSCLLCAT)': 'OV-Best Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best OV Time Point'}, inplace=True)
        # Merge left with the primary current response dataframe
        final_responseB_primary_df = pd.merge(responseB_primary_current_df,responseB_best_OV_df, on='Subject', how='left')
        ...
        responseB_best_CT_df = responseB_best_CT_df[['Subject', 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column CT-Based CLL Disease Response to CT-Based Response
        responseB_best_CT_df.rename(columns={'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)': 'BM-Best Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best BM Time Point'}, inplace=True)
        # Merge left with the primary response dataframe
        final_responseB_primary_df = pd.merge(final_responseB_primary_df, responseB_best_CT_df, on='Subject', how='left')
        ...
        responseB_primary_M3_df = responseB_primary_M3_df[['Subject', 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)', 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']]
        ...
        responseB_primary_M3_df = pd.concat([responseB_primary_M3_df, responseB_df[~responseB_df['Subject'].isin(responseB_primary_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
        ...
        temp_df = temp_df[temp_df['Overall CLL Disease Response (ig_RS2.RSCLLCAT)'].notna() | temp_df['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'].notna()]
        ...
        for index, row in responseB_primary_M3_df.iterrows():
            ...
            if pd.isna(row['Overall CLL Disease Response (ig_RS2.RSCLLCAT)']) and pd.isna(row['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']):
                # if yes, check to see if the subject in in PD_df
                if row['Subject'] in PD_df['Subject'].values:
                    # get the End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT) date of the subject in PD_df
                    end_date = PD_df[PD_df['Subject'] == row['Subject']]['End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)'].values[0]
                    ...
                    if not filtered_df.empty:
                        ...
                        responseB_primary_M3_df.loc[index, 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)'] = temp_row['Overall CLL Disease Response (ig_RS2.RSCLLCAT)']
                        ...
                        responseB_primary_M3_df.loc[index, 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'] = temp_row['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']
                # check if the subject is in Initiation of REtx before month 3
                elif row['Subject'] in DSINITRT_df['Subject'].values:
                    # get the End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT) of the subject in DSINITRT_df
                    end_date = DSINITRT_df[DSINITRT_df['Subject'] == row['Subject']]['End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT)'].values[0]
                    # if yes, find the response of the same subject with the latest event date that is before the Month 3 event date
                    filtered_df = temp_df[(temp_df['Subject'] == row['Subject'])  & (temp_df['Event Date'] <= end_date)]
                    # Check if the filtered DataFrame is empty
                    if not filtered_df.empty:
        ...
        responseB_primary_M3_df.rename(columns={'Overall CLL Disease Response (ig_RS2.RSCLLCAT)': 'Overall Response', 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)': 'Bone Marrow Response'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseB_primary_df = pd.merge(final_responseB_primary_df, responseB_primary_M3_df, on='Subject', how='left')
        ...
        # Getting AE and SAE dataframes
        responseB_primary_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseB_primary_df['AE'] = final_responseB_primary_df['Subject'].apply(lambda x: 'Y' if x in responseB_primary_AE_df['Subject'].values else 'N')
        ...
        final_responseB_primary_df['SAE'] = final_responseB_primary_df['Subject'].apply(lambda x: 'Y' if x in responseB_primary_AE_df[responseB_primary_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
        ...
        # Merge left with the current response dataframe
        final_responseB_primary_df = pd.merge(final_responseB_primary_df, responseB_primary_SV_df[['Subject', 'Event Label']], on='Subject', how='left')
        # Select the columns needed only
        final_responseB_primary_df = final_responseB_primary_df[['Subject',
    ...
    # Convert the column For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR) to string
    responseB_retreatment_df = convert_integers_to_strings(responseB_retreatment_df, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)')
    responseB_retreatment_df.loc[temp_mask, 'Retreatment Time Point (ig_RS1.RSTPTR)'] = "Day " + responseB_retreatment_df.loc[temp_mask, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseB_retreatment_df = responseB_retreatment_df[responseB_retreatment_df['Retreatment Time Point (ig_RS1.RSTPTR)'] != 'Pre-Retreatment Safety Visit']
    ...
    if final_subject_B_retx_count > 0:
        ...
        # Convert OV-Based CLL Disease Response and CT-Based CLL Disease Response to numeric values
        responseB_retreatment_df['OV-Score'] = responseB_retreatment_df['Overall CLL Disease Response (ig_RS2.RSCLLCAT)'].map(DR_CLL_OV_dict)
        responseB_retreatment_df['BM-Score'] = responseB_retreatment_df['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'].map(DR_CLL_BM_dict)
        ...
        # Fill NaN with "Not Reported" in column Overall CLL Disease Response (ig_RS2.RSCLLCAT)
        responseB_retreatment_current_df['Overall CLL Disease Response (ig_RS2.RSCLLCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)
        responseB_retreatment_current_df['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'].fillna('Not Reported', inplace=True)
        ...
        responseB_best_OV_df.rename(columns={'Overall CLL Disease Response (ig_RS2.RSCLLCAT)': 'OV-Best Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best OV Time Point'}, inplace=True)
        # Merge left with the primary current response dataframe
        final_responseB_retreatment_df = pd.merge(responseB_retreatment_current_df,responseB_best_OV_df, on='Subject', how='left')
        ...
        responseB_best_CT_df.rename(columns={'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)': 'BM-Best Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best BM Time Point'}, inplace=True)
        # Merge left with the primary response dataframe
        final_responseB_retreatment_df = pd.merge(final_responseB_retreatment_df, responseB_best_CT_df, on='Subject', how='left')
        

        ## * Overall CLL Disease Response at Month 3 primary
        # Filter responseB_retreatment_df to only Month 3
        responseB_retreatment_M3_df = responseB_df[responseB_df['Primary Treatment Time Point (ig_RS1.RSTPT)'] == 'Month 3']
        # Selec the columns subject and Overall CLL Disease Response (ig_RS2.RSCLLCAT) from responseB_retreatment_M3_df
        responseB_retreatment_M3_df = responseB_retreatment_M3_df[['Subject', 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)', 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']]
        ...
        responseB_retreatment_M3_df = pd.concat([responseB_retreatment_M3_df, responseB_df[~responseB_df['Subject'].isin(responseB_retreatment_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
            ...
            if pd.isna(row['Overall CLL Disease Response (ig_RS2.RSCLLCAT)']) and pd.isna(row['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']):
                ...
                if row['Subject'] in PD_Retx_df['Subject'].values:
                    ...
                    if not filtered_df.empty:
                        ...
                        temp_row = filtered_df.iloc[-1]
                        # Replace the Overall CLL Disease Response (ig_RS2.RSCLLCAT) of the subject with the Overall CLL Disease Response (ig_RS2.RSCLLCAT) of the temp_row
                        responseB_retreatment_M3_df.loc[index, 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)'] = temp_row['Overall CLL Disease Response (ig_RS2.RSCLLCAT)']
                        # Replace the CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP) of the subject with the CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP) of the temp_row
                        responseB_retreatment_M3_df.loc[index, 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'] = temp_row['CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)']
        # Rename the column CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP) to OV-Based ORR
        responseB_retreatment_M3_df.rename(columns={'Overall CLL Disease Response (ig_RS2.RSCLLCAT)': 'Overall Response', 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)': 'Bone Marrow Response'}, inplace=True)                    
        # Merge left with the current response dataframe
        final_responseB_retreatment_df = pd.merge(final_responseB_retreatment_df, responseB_retreatment_M3_df, on='Subject', how='left')
        ...
        ## Checking AE and SAE for CLL primary
        # Getting AE and SAE dataframes
        responseB_retreatment_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseB_retreatment_df['AE'] = final_responseB_retreatment_df['Subject'].apply(lambda x: 'Y' if x in responseB_retreatment_AE_df['Subject'].values else 'N')
        ...
        final_responseB_retreatment_df['SAE'] = final_responseB_retreatment_df['Subject'].apply(lambda x: 'Y' if x in responseB_retreatment_AE_df[responseB_retreatment_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
        ...
        # Merge left with the current response dataframe
        final_responseB_retreatment_df = pd.merge(final_responseB_retreatment_df, responseB_retreatment_SV_df[['Subject', 'Event Label']], on='Subject', how='left')
        ...
        # Select the columns needed only
        final_responseB_retreatment_df = final_responseB_retreatment_df[['Subject',
                                                    'Overall CLL Disease Response (ig_RS2.RSCLLCAT)',
                                                    'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)',
                                                    'Primary Treatment Time Point (ig_RS1.RSTPT)',
                                                    'OV-Best Response',
    ...
    # Convert the column For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY) to string
    responseBRT_primary_df = convert_integers_to_strings(responseBRT_primary_df, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)')
    responseBRT_primary_df.loc[temp_mask, 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = "Day " + responseBRT_primary_df.loc[temp_mask, 'For Unscheduled Primary Treatment Time Point, Specify Day #  (ig_RS1.RSTPTDY)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseBRT_primary_df = responseBRT_primary_df[responseBRT_primary_df['Primary Treatment Time Point (ig_RS1.RSTPT)'] != 'Pre-Treatment Safety Visit']
    ...
    if final_subject_BRT_prim_count > 0:
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseBRT_primary_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseBRT_primary_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        # Convert PET-Based NHL Disease Response and CT-Based NHL Disease Response to numeric values
        responseBRT_primary_df['PET-Score'] = responseBRT_primary_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].map(DR_NHL_PET_dict)
        responseBRT_primary_df['CT-Score'] = responseBRT_primary_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].map(DR_NHL_CT_dict)
        ...
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseBRT_primary_current_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseBRT_primary_current_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        ...
        responseBRT_best_PET_df = responseBRT_best_PET_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column PET-Based NHL Disease Response to PET-Based Response
        responseBRT_best_PET_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best PET Time Point'}, inplace=True)
        # Merge left with the primary current response dataframe
        final_responseBRT_primary_df = pd.merge(responseBRT_primary_current_df,responseBRT_best_PET_df, on='Subject', how='left')
        ...
        responseBRT_best_CT_df = responseBRT_best_CT_df[['Subject', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Primary Treatment Time Point (ig_RS1.RSTPT)']]
        # Rename the column CT-Based NHL Disease Response to CT-Based Response
        responseBRT_best_CT_df.rename(columns={'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based Response', 'Primary Treatment Time Point (ig_RS1.RSTPT)' : 'Best CT Time Point'}, inplace=True)
        # Merge left with the primary response dataframe
        final_responseBRT_primary_df = pd.merge(final_responseBRT_primary_df, responseBRT_best_CT_df, on='Subject', how='left')
        

        ## Overall NHL Disease Response at Month 3 primary
        # Filter responseBRT_primary_df to only Month 3
        responseBRT_primary_M3_df = responseBRT_df[responseBRT_df['Primary Treatment Time Point (ig_RS1.RSTPT)'] == 'Month 3']
        # Selec the columns subject and PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) from responseBRT_primary_M3_df
        responseBRT_primary_M3_df = responseBRT_primary_M3_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']]
        ...
        responseBRT_primary_M3_df = pd.concat([responseBRT_primary_M3_df, responseBRT_df[~responseBRT_df['Subject'].isin(responseBRT_primary_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
            ...
            if pd.isna(row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']) and pd.isna(row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']):
                ...
                if row['Subject'] in PD_df['Subject'].values:
                    ...
                    if not filtered_df.empty:
                        ...
                        temp_row = filtered_df.iloc[-1]
                        # Replace the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the subject with the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the temp_row
                        responseBRT_primary_M3_df.loc[index, 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = temp_row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']
                        # Replace the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the subject with the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the temp_row
                        responseBRT_primary_M3_df.loc[index, 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = temp_row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']
                # check if the subject is in Initiation of REtx before month 3
                elif row['Subject'] in DSINITRT_df['Subject'].values:
                    # get the End of Primary Follow-Up Date (ig_INITLF1.DSENPFUDAT) date of the subject in DSINITRT_df
                    end_date = DSINITRT_df[DSINITRT_df['Subject'] == row['Subject']]['End of Primary Follow-Up Date (ig_DSINITRT1.DSRENPFUDAT)'].values[0]
                    ...
                    if not filtered_df.empty:
                        ...
                        responseBRT_primary_M3_df.loc[index, 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = temp_row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']
        # Rename the column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) to PET-Based ORR
        responseBRT_primary_M3_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based ORR', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based ORR'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseBRT_primary_df = pd.merge(final_responseBRT_primary_df, responseBRT_primary_M3_df, on='Subject', how='left')
        ...
        ## Checking AE and SAE for NHL primary
        # Getting AE and SAE dataframes
        responseBRT_primary_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseBRT_primary_df['AE'] = final_responseBRT_primary_df['Subject'].apply(lambda x: 'Y' if x in responseBRT_primary_AE_df['Subject'].values else 'N')
        ...
        final_responseBRT_primary_df['SAE'] = final_responseBRT_primary_df['Subject'].apply(lambda x: 'Y' if x in responseBRT_primary_AE_df[responseBRT_primary_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
        ...
        # Merge left with the current response dataframe
        final_responseBRT_primary_df = pd.merge(final_responseBRT_primary_df, responseBRT_primary_SV_df[['Subject', 'Event Label']], on='Subject', how='left')
        # Rename the column Event Label to Event Label (Study Status)
        final_responseBRT_primary_df['Event Label'] = final_responseBRT_primary_df['Event Label'].map(event_AB_dict)

        # Select the columns needed only
        final_responseBRT_primary_df = final_responseBRT_primary_df[['Subject',
                                                    'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)',
                                                    'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)',
                                                    'Primary Treatment Time Point (ig_RS1.RSTPT)',
                                                    'PET-Based Response',
    ...
    # Convert the column For Unscheduled Retreatment Time Point, Specify Day #  (ig_RS1.UNSDAYR) to string
    responseBRT_retreatment_df = convert_integers_to_strings(responseBRT_retreatment_df, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)')
    responseBRT_retreatment_df.loc[temp_mask, 'Retreatment Time Point (ig_RS1.RSTPTR)'] = "Day " + responseBRT_retreatment_df.loc[temp_mask, 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RS1.UNSDAYR)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseBRT_retreatment_df = responseBRT_retreatment_df[responseBRT_retreatment_df['Retreatment Time Point (ig_RS1.RSTPTR)'] != 'Pre-Retreatment Safety Visit']
    ...
    if final_subject_BRT_retx_count > 0:
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseBRT_retreatment_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseBRT_retreatment_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        # Convert PET-Based NHL Disease Response and CT-Based NHL Disease Response to numeric values
        responseBRT_retreatment_df['PET-Score'] = responseBRT_retreatment_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].map(DR_NHL_PET_dict)
        responseBRT_retreatment_df['CT-Score'] = responseBRT_retreatment_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].map(DR_NHL_CT_dict)
        ...
        # Fill NaN with "Not Reported" in column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)
        responseBRT_retreatment_current_df['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'].fillna('Not Reported', inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseBRT_retreatment_current_df['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'].fillna('Not Reported', inplace=True)
        ...
        # Select the columns subject and PET-Based NHL Disease Response from responseBRT_best_PET_df
        responseBRT_best_PET_df = responseBRT_best_PET_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'Retreatment Time Point (ig_RS1.RSTPTR)']]
        # Rename the column PET-Based NHL Disease Response to PET-Based Response
        responseBRT_best_PET_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based Response', 'Retreatment Time Point (ig_RS1.RSTPTR)' : 'Best PET Time Point'}, inplace=True)
        # Fill NaN with "Not Reported" in column PET-Based Response
        responseBRT_best_PET_df['PET-Based Response'].fillna('Not Reported', inplace=True)
        # Merge left with the primary current response dataframe
        final_responseBRT_retreatment_df = pd.merge(responseBRT_retreatment_current_df,responseBRT_best_PET_df, on='Subject', how='left')
        ...
        # Select the columns subject and CT-Based NHL Disease Response from responseBRT_best_CT_df
        responseBRT_best_CT_df = responseBRT_best_CT_df[['Subject', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)', 'Retreatment Time Point (ig_RS1.RSTPTR)']]
        # Rename the column CT-Based NHL Disease Response to CT-Based Response
        responseBRT_best_CT_df.rename(columns={'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based Response', 'Retreatment Time Point (ig_RS1.RSTPTR)' : 'Best CT Time Point'}, inplace=True)
        # Fill NaN with "Not Reported" in column CT-Based Response
        responseBRT_best_CT_df['CT-Based Response'].fillna('Not Reported', inplace=True)
        # Merge left with the primary response dataframe
        final_responseBRT_retreatment_df = pd.merge(final_responseBRT_retreatment_df, responseBRT_best_CT_df, on='Subject', how='left')
        

        ## * Overall NHL Disease Response at Month 3 R
        # Filter responseBRT_retreatment_df to only Month 3
        responseBRT_retreatment_M3_df = responseBRT_df[responseBRT_df['Retreatment Time Point (ig_RS1.RSTPTR)'] == 'Month 3']
        # Selec the columns subject and PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) from responseBRT_retreatment_M3_df
        responseBRT_retreatment_M3_df = responseBRT_retreatment_M3_df[['Subject', 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']]
        ...
        responseBRT_retreatment_M3_df = pd.concat([responseBRT_retreatment_M3_df, responseBRT_df[~responseBRT_df['Subject'].isin(responseBRT_retreatment_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
            ...
            if pd.isna(row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']) and pd.isna(row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']):
                ...
                if row['Subject'] in PD_Retx_df['Subject'].values:
                    ...
                    if not filtered_df.empty:
                        ...
                        temp_row = filtered_df.iloc[-1]
                        # Replace the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the subject with the PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) of the temp_row
                        responseBRT_retreatment_M3_df.loc[index, 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = temp_row['PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)']
                        # Replace the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the subject with the CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT) of the temp_row
                        responseBRT_retreatment_M3_df.loc[index, 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = temp_row['CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)']
        # Rename the column PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT) to PET-Based ORR
        responseBRT_retreatment_M3_df.rename(columns={'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)': 'PET-Based ORR', 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)': 'CT-Based ORR'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseBRT_retreatment_df = pd.merge(final_responseBRT_retreatment_df, responseBRT_retreatment_M3_df, on='Subject', how='left')
        ...
        ## Checking AE and SAE for NHL primary
        # Getting AE and SAE dataframes
        responseBRT_retreatment_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseBRT_retreatment_df['AE'] = final_responseBRT_retreatment_df['Subject'].apply(lambda x: 'Y' if x in responseBRT_retreatment_AE_df['Subject'].values else 'N')
        ...
        final_responseBRT_retreatment_df['SAE'] = final_responseBRT_retreatment_df['Subject'].apply(lambda x: 'Y' if x in responseBRT_retreatment_AE_df[responseBRT_retreatment_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
        ...
        # Merge left with the current response dataframe
        final_responseBRT_retreatment_df = pd.merge(final_responseBRT_retreatment_df, responseBRT_retreatment_SV_df[['Subject', 'Event Label']], on='Subject', how='left')
        
        # * Formatting the dataframe
        # Rename the column Event Label to Event Label (Study Status)
        final_responseBRT_retreatment_df['Event Label'] = final_responseBRT_retreatment_df['Event Label'].map(event_AB_dict)
        
        # Select the columns needed only
        final_responseBRT_retreatment_df = final_responseBRT_retreatment_df[['Subject',
                                                    'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)',
                                                    'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)',
                                                    'Retreatment Time Point (ig_RS1.RSTPTR)',
                                                    'PET-Based Response',
    ...
    # Response data dataframe for NHL only
    responseC_df = data['RSALL'][['Subject', 'Event Date','Study Phase (ig_RSALL1.STUDYPHS2)', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)', 'For Unscheduled Primary Treatment Time Point, Specify Day # (ig_RSALL1.UNSDAY)',  'Retreatment Time Point (ig_RSALL1.RSALLTPTR)', 'For Unscheduled Retreatment Time Point, Specify Day # (ig_RSALL1.UNSDAYR)', 'Overall Disease Response (ig_RSALL2.RSALLCAT)','Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)']].copy()
    ...
    responseC_primary_df = convert_integers_to_strings(responseC_primary_df, 'For Unscheduled Primary Treatment Time Point, Specify Day # (ig_RSALL1.UNSDAY)')
    responseC_primary_df.loc[temp_mask, 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)'] = "Day " + responseC_primary_df.loc[temp_mask, 'For Unscheduled Primary Treatment Time Point, Specify Day # (ig_RSALL1.UNSDAY)'].astype(str)
    # Remove rows with Pre-Treatment Safety Visit
    responseC_primary_df = responseC_primary_df[responseC_primary_df['Primary Treatment Time Point (ig_RSALL1.RSALLTPT)'] != 'Pre-Treatment Safety Visit']
    # Remove rows with Post-Lymphodepleting Chemotherapy
    responseC_primary_df = responseC_primary_df[responseC_primary_df['Primary Treatment Time Point (ig_RSALL1.RSALLTPT)'] != 'Post-Lymphodepleting Chemotherapy']
    # replace all rows with Extramedullary Disease Without Bone Marrow Involvement to Not Applicable
    responseC_primary_df['Overall Disease Response (ig_RSALL2.RSALLCAT)'] = responseC_primary_df['Overall Disease Response (ig_RSALL2.RSALLCAT)'].replace('Extramedullary Disease Without Bone Marrow Involvement', 'Not Applicable')
    ...
    if final_subject_C_prim_count > 0:
        ...
        responseC_primary_df['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)'].fillna('Not Applicable', inplace=True)
        # Convert PET-Based NHL Disease Response and CT-Based NHL Disease Response to numeric values
        responseC_primary_df['OV-Score'] = responseC_primary_df['Overall Disease Response (ig_RSALL2.RSALLCAT)'].map(DR_ALL_OV_dict)
        responseC_primary_df['ED-Score'] = responseC_primary_df['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)'].map(DR_ALL_ED_dict)
        ...
        responseC_primary_current_df['Overall Disease Response (ig_RSALL2.RSALLCAT)'].fillna('Not Applicable', inplace=True)
        # Fill NaN with "Not Applicable" in column CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)
        responseC_primary_current_df['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)'].fillna('Not Applicable', inplace=True)
        # rename the column Overall Disease Response (ig_RSALL2.RSALLCAT) to Current Response
        responseC_primary_current_df.rename(columns={'Overall Disease Response (ig_RSALL2.RSALLCAT)': 'Current Overall Response', 'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)': 'Current ED Response', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)' : 'Current Response Time Point'}, inplace=True)
        ...
        responseC_best_OV_df = responseC_best_OV_df[['Subject', 'Overall Disease Response (ig_RSALL2.RSALLCAT)', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)']]
        # Rename the column OV-Based NHL Disease Response to OV-Based Response
        responseC_best_OV_df.rename(columns={'Overall Disease Response (ig_RSALL2.RSALLCAT)': 'Best Overall Response', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)' : 'Best Overall Response Time Point'}, inplace=True)
        #Replace Nan with Not Applicable
        responseC_best_OV_df['Best Overall Response'].fillna('Not Applicable', inplace=True)
        # Merge left with the primary current response dataframe
        final_responseC_primary_df = pd.merge(responseC_primary_current_df,responseC_best_OV_df, on='Subject', how='left')
        ...
        responseC_best_ED_df = responseC_best_ED_df[['Subject', 'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)']]
        # Rename the column CT-Based NHL Disease Response to CT-Based Response
        responseC_best_ED_df.rename(columns={'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)': 'Best ED Response', 'Primary Treatment Time Point (ig_RSALL1.RSALLTPT)' : 'Best ED Time Point'}, inplace=True)
        #Replace Nan with Not Applicable
        responseC_best_ED_df['Best ED Response'].fillna('Not Applicable', inplace=True)
        # Merge left with the primary response dataframe
        final_responseC_primary_df = pd.merge(final_responseC_primary_df, responseC_best_ED_df, on='Subject', how='left')
        

        ## * Overall ALL Disease Response at Day 28 primary
        # Filter responseC_primary_df to only Month 3
        responseC_primary_M3_df = responseC_df[responseC_df['Primary Treatment Time Point (ig_RSALL1.RSALLTPT)'] == 'Day 28']
        # Selec the columns subject and OV, ED from responseC_primary_M3_df
        responseC_primary_M3_df = responseC_primary_M3_df[['Subject', 'Overall Disease Response (ig_RSALL2.RSALLCAT)', 'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)']]
        ...
        responseC_primary_M3_df = pd.concat([responseC_primary_M3_df, responseC_df[~responseC_df['Subject'].isin(responseC_primary_M3_df['Subject'])][['Subject']]])
        # Remove duplicates
        ...
        temp_df = temp_df[temp_df['Overall Disease Response (ig_RSALL2.RSALLCAT)'].notna() | temp_df['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)'].notna()]
        ...
        for index, row in responseC_primary_M3_df.iterrows():
            ...
            if pd.isna(row['Overall Disease Response (ig_RSALL2.RSALLCAT)']) and pd.isna(row['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)']):
                # if yes, check to see if the subject in in PD_df
                if row['Subject'] in PD_df['Subject'].values:
                    ...
                    if not filtered_df.empty:
                        ...
                        responseC_primary_M3_df.loc[index, 'Overall Disease Response (ig_RSALL2.RSALLCAT)'] = temp_row['Overall Disease Response (ig_RSALL2.RSALLCAT)']
                        ...
                        responseC_primary_M3_df.loc[index, 'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)'] = temp_row['Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)']
                # check if the subject is in Initiation of REtx before month 3
                elif row['Subject'] in DSINITRT_df['Subject'].values:
        ...
        responseC_primary_M3_df.rename(columns={'Overall Disease Response (ig_RSALL2.RSALLCAT)': 'OV ORR', 'Radiographic Tumor Response for	Extramedullary Disease (ig_RSALL2.RADTURESEXDS)': 'ED ORR'}, inplace=True)
        # Merge left with the current response dataframe
        final_responseC_primary_df = pd.merge(final_responseC_primary_df, responseC_primary_M3_df, on='Subject', how='left')
        ...
        final_responseC_primary_df['OV ORR'] = final_responseC_primary_df['OV ORR'].replace('Extramedullary Disease Without Bone Marrow Involvement', 'Not Applicable')
        
        ## Checking AE and SAE for NHL primary
        # Getting AE and SAE dataframes
        responseC_primary_AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']]
        ...
        final_responseC_primary_df['AE'] = final_responseC_primary_df['Subject'].apply(lambda x: 'Y' if x in responseC_primary_AE_df['Subject'].values else 'N')
        ...
        final_responseC_primary_df['SAE'] = final_responseC_primary_df['Subject'].apply(lambda x: 'Y' if x in responseC_primary_AE_df[responseC_primary_AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
        ...
        # Merge left with the current response dataframe
        final_responseC_primary_df = pd.merge(final_responseC_primary_df, responseC_primary_SV_df[['Subject', 'Event Label']], on='Subject', how='left')
        # Rename the column Event Label to Event Label (Study Status)
        final_responseC_primary_df['Event Label'] = final_responseC_primary_df['Event Label'].map(event_C_dict)

        # Select the columns needed only
        final_responseC_primary_df = final_responseC_primary_df[['Subject',
        ...
        final_subject_C_prim_OR_count = len(final_responseC_primary_df[final_responseC_primary_df['Best Overall Response'] != 'Not Applicable']['Subject'].unique())
        # Count number of subject of column 'Best ED Response' that is not 'Not Applicable'
        final_subject_C_prim_ED_count = len(final_responseC_primary_df[final_responseC_primary_df['Best ED Response'] != 'Not Applicable']['Subject'].unique())
    ### TODO: REPONSE STATS
    # TODO: SAFETY STATS
    ...
    # Getting AE and SAE dataframes
    AE_df = data['AE'][['Subject', 'Form ILB Status', 'AE or SAE? (ig_AE2.AESEV)', 'Event Onset (ig_AE1.AEONSET)']].copy()
    # Check responseA_primary_AE_df if the subject of responseA_primary_df is in the AE dataframe. If yes, then add 'Y' to the column 'AE' in responseA_primary_df, else add 'N'
    total_infused_df['AE'] = total_infused_df['Subject'].apply(lambda x: 'Y' if x in AE_df['Subject'].values else 'N')
    # Check responseA_primary_AE_df if the subject of responseA_primary_df has SAE in column 'AE or SAE? (ig_AE2.AESEV)' . If yes, then add 'Y' to the column 'SAE' in responseA_primary_df, else add 'N'
    total_infused_df['SAE'] = total_infused_df['Subject'].apply(lambda x: 'Y' if x in AE_df[AE_df['AE or SAE? (ig_AE2.AESEV)'] == 'SAE']['Subject'].values else 'N')
    ...
    AE_A_count = get_stats_percentage('AE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort A: Non-Hodgkin Lymphoma (NHL)']).T
    SAE_A_count = get_stats_percentage('SAE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort A: Non-Hodgkin Lymphoma (NHL)']).T
    # merge AE and SAE dataframes
    final_safety_A_df = pd.concat([AE_A_count, SAE_A_count], axis=1)
    
    AE_B_count = get_stats_percentage('AE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort B: Chronic Lymphocytic Leukemia (CLL)']).T
    SAE_B_count = get_stats_percentage('SAE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort B: Chronic Lymphocytic Leukemia (CLL)']).T
    # merge AE and SAE dataframes
    final_safety_B_df = pd.concat([AE_B_count, SAE_B_count], axis=1)
    
    AE_C_count = get_stats_percentage('AE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort C: Acute Lymphoblastic Leukemia (ALL)']).T
    SAE_C_count = get_stats_percentage('SAE', total_infused_df[total_infused_df['Cohort Assignment'] == 'Cohort C: Acute Lymphoblastic Leukemia (ALL)']).T
    # merge AE and SAE dataframes
    final_safety_C_df = pd.concat([AE_C_count, SAE_C_count], axis=1)
    
    final_safety_df = pd.concat([final_safety_total_df, final_safety_A_df, final_safety_B_df, final_safety_C_df], axis=0)
    ...
    if final_subject_C_prim_count > 0:
        ...
        responseC_stat = responseC_stat.replace('Extramedullary Disease Without Bone Marrow Involvement', 'Not Applicable')
    ...
    if final_subject_A_prim_count > 0:
        final_responseA_primary_df.loc[(final_responseA_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseA_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseA_primary_df['Event Label'] == 'Pre-Retreatment'), 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = 'Transitioned to Retreatment'
        final_responseA_primary_df.loc[(final_responseA_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseA_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseA_primary_df['Event Label'] == 'Pre-Retreatment'), 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = 'Transitioned to Retreatment'
        final_responseA_primary_df.loc[(final_responseA_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseA_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseA_primary_df['Event Label'] == 'Pre-Retreatment'), 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = 'Transitioned to Retreatment'

    if final_subject_B_prim_count > 0:
        final_responseB_primary_df.loc[(final_responseB_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseB_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseB_primary_df['Event Label'] == 'Pre-Retreatment'), 'Overall CLL Disease Response (ig_RS2.RSCLLCAT)'] = 'Transitioned to Retreatment'
        final_responseB_primary_df.loc[(final_responseB_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseB_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseB_primary_df['Event Label'] == 'Pre-Retreatment'), 'CLL Bone Marrow Response (ig_RS2.RSCLLBMRESP)'] = 'Transitioned to Retreatment'
        final_responseB_primary_df.loc[(final_responseB_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseB_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseB_primary_df['Event Label'] == 'Pre-Retreatment'), 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = 'Transitioned to Retreatment'

    if final_subject_C_prim_count > 0:
        final_responseC_primary_df.loc[(final_responseC_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseC_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseC_primary_df['Event Label'] == 'Pre-Retreatment'), 'Current Overall Response'] = 'Transitioned to Retreatment'
        final_responseC_primary_df.loc[(final_responseC_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseC_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseC_primary_df['Event Label'] == 'Pre-Retreatment'), 'Current ED Response'] = 'Transitioned to Retreatment'
        final_responseC_primary_df.loc[(final_responseC_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseC_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseC_primary_df['Event Label'] == 'Pre-Retreatment'), 'Current Response Time Point'] = 'Transitioned to Retreatment'

    if final_subject_BRT_prim_count > 0:
        final_responseBRT_primary_df.loc[(final_responseBRT_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseBRT_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseBRT_primary_df['Event Label'] == 'Pre-Retreatment'), 'PET-Based NHL Disease Response (ig_RS3.RSNHLPETCAT)'] = 'Transitioned to Retreatment'
        final_responseBRT_primary_df.loc[(final_responseBRT_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseBRT_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseBRT_primary_df['Event Label'] == 'Pre-Retreatment'), 'CT-Based NHL Disease Response (ig_RS3.RSNHLCTCAT)'] = 'Transitioned to Retreatment'
        final_responseBRT_primary_df.loc[(final_responseBRT_primary_df['Event Label'] == 'Retreatment Long Term Follow-up') | (final_responseBRT_primary_df['Event Label'] == 'Primary Retreatment') | (final_responseBRT_primary_df['Event Label'] == 'Pre-Retreatment'), 'Primary Treatment Time Point (ig_RS1.RSTPT)'] = 'Transitioned to Retreatment'
    ...
    if export:
        ...
        with pd.ExcelWriter(output_dir  + '/' + output_file_name + '.xlsx', engine = 'xlsxwriter') as writer:  
            ...
            final_infusion_statA.to_excel(writer, sheet_name='DSMB-New Infusion Statistics', index = False, startrow=2, startcol=0)
            final_infusion_statB.to_excel(writer, sheet_name='DSMB-New Infusion Statistics', index = False, startrow=6, startcol=0)
            final_infusion_statC.to_excel(writer, sheet_name='DSMB-New Infusion Statistics', index = False, startrow=10, startcol=0)
            ...
            if final_subject_A_prim_count > 0:
                final_response_stat_A_BOR_PET.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=10, startcol=1)
                final_response_stat_A_BOR_CT.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=10, startcol=3)
                final_response_stat_A_ORR_PET.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=17, startcol=1)
                final_response_stat_A_ORR_CT.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=17, startcol=3)
            # Resposne Data Cohort B
            if final_subject_B_prim_count > 0:
                final_response_stat_B_BOR_OV.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=10, startcol=5)
                final_response_stat_B_BOR_BM.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=10, startcol=7)
                final_response_stat_B_ORR_OV.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=17, startcol=5)
                final_response_stat_B_ORR_BM.to_excel(writer, sheet_name='DSMB-Response Stats', index = False, startrow=17, startcol=7)
            ...
            worksheet5.merge_range('B1:E1', 'Safety Statistics (N=' + str(safety_total_df_subject_count) + ')', bold_12_wrap_format)
            ...
            # Response Headers
            worksheet5.merge_range('A9:D9', 'NHL Subject Response (N=' + str(final_subject_A_prim_count) + ')', bold_12_format)
            worksheet5.merge_range('E9:H9', 'CLL Subject Response (N=' + str(final_subject_B_prim_count) + ')', bold_12_format)
            worksheet5.merge_range('I9:L9', 'ALL Subject Response', bold_12_format)
            worksheet5.merge_range('A26:D26', 'Richter\'s Transformation Subject Response (N=' + str(final_subject_BRT_prim_count) + ')', bold_12_format)
            ...
            if final_subject_C_prim_count > 0:
                worksheet5.merge_range('I10:J10', 'Overall Response (N=' + str(final_subject_C_prim_OR_count) + ')' , bold_11_format)
                worksheet5.merge_range('K10:L10', 'Extramedullary Disease without Bone Marrow Involvement (N=' + str(final_subject_C_prim_ED_count) + ')', bold_11_format)
            else:
                worksheet5.merge_range('I10:J10', 'Overall Response (N=0)' , bold_11_format)
                worksheet5.merge_range('K10:L10', 'Extramedullary Disease without Bone Marrow Involvement (N=0)', bold_11_format)
            ...
            resposne_B_OV = ['Complete Remission (CR)', 'Complete Remission with Incomplete Marrow Recovery (CRi)', 'Partial Remission (PR)', 'Stable Disease (SD)', 'Progressive Disease (PD)', 'Not Reported' ]
            response_B_BM = ['Complete Remission (CR)', 'Partial Remission (PR)', 'Progressive Disease (PD)', 'Stable Disease (SD)', 'Not Reported']
            response_C_OV = ['Complete Remission (CR)', 'Complete Remission with Incomplete Blood Count Recovery (CRi)', 'Complete Remission with Residual Mediastinal Disease (CRu)', 'Treatment Failure (TF)', 'Relapsed Disease (RD)', 'Not Reported' ]
            response_C_ED = ['Complete Remission (CR)', 'Partial Remission (PR)', 'Stable Disease (SD)', 'Indeterminate Response', 'Progressive Disease (PD)', 'Not Reported' ]
            ...
            # * WRITING DATA
            if final_subject_A_prim_count > 0:
                final_responseA_primary_df.to_excel(writer, sheet_name='Response Listing NHL', index = False, header=False , startrow=3, startcol=0)
            if final_subject_A_retx_count > 0:
                final_responseA_retreatment_df.to_excel(writer, sheet_name='Response Listing NHL', index = False, header=False,  startrow=3, startcol=15)
            ...
            # * WRITING DATA
            if final_subject_B_prim_count > 0:
                final_responseB_primary_df.to_excel(writer, sheet_name='Response Listing CLL', index = False, startrow=2, startcol=0)
            ...
            # * WRITING DATA
            if final_subject_BRT_prim_count > 0:
                final_responseBRT_primary_df.to_excel(writer, sheet_name='Response Listing Richter', index = False, header=False , startrow=3, startcol=0)
                # assigning the worksheet
                ...
                worksheet8.autofit()
            if final_subject_BRT_retx_count > 0:
                final_responseBRT_retreatment_df.to_excel(writer, sheet_name='Response Listing Richter', index = False, header=False,  startrow=3, startcol=15)
            ...
            # Autofit
            if final_subject_C_prim_count > 0:
                final_responseC_primary_df.to_excel(writer, sheet_name='Response Listing ALL', index = False, startrow=2, startcol=0)
