...
def get_stats_percentage(column, *args):
    ...
    """
    if column == 'Legal Sex':
        groupby = ['Male', 'Female', 'Nonbinary (X)', 'Not Reported']
    elif column == 'Race':
        groupby = ['African American', 'Alaska Native', 'American Indian', 'Asian','Caucasian', 'Multiple Races', 'Pacific Islander', 'Other', 'Unknown']
    ...
    # NHL
        ...
        groupby = ['Complete Metabolic Response (CMR)', 'Partial Metabolic Response (PMR)', 'No Metabolic Response (NMR)', 'Indeterminate Response (IR)', 'Progressive Metabolic Disease (PMD)', 'Not Reported' ]
    elif column == 'CT-Based Response'or column == 'CT-Based ORR':
        groupby = ['Complete Radiologic Response (CR)', 'Partial Response (PR)', 'Stable Disease (SD)', 'Indeterminate Response (IR)', 'Progressive Disease (PD)', 'Not Reported' ]
    # CLL
    elif column == 'OV-Best Response' or column == 'Overall Response':
        groupby = ['Complete Remission (CR)', 'Complete Remission with Incomplete Marrow Recovery (CRi)', 'Partial Remission (PR)', 'Stable Disease (SD)', 'Progressive Disease (PD)', 'Not Reported' ]
    elif column == 'BM-Best Response' or column == 'Bone Marrow Response':
        groupby = ['Complete Remission (CR)', 'Partial Remission (PR)', 'Progressive Disease (PD)', 'Stable Disease (SD)', 'Not Reported' ]
    # ALL
    elif column == 'Best Overall Response' or column == 'OV ORR':
        groupby = ['Complete Remission (CR)','Complete Remission with Incomplete Blood Count Recovery (CRi)', 'Complete Remission with Residual Mediastinal Disease (CRu)', 'Treatment Failure (TF)', 'Relapsed Disease (RD)', 'Not Reported']
    elif column == 'Best ED Response' or column == 'ED ORR':
        groupby = ['Complete Remission (CR)', 'Partial Remission (PR)', 'Stable Disease (SD)', 'Indeterminate Response', 'Progressive Disease (PD)', 'Not Reported' ]
    else:
        groupby = None
    ...
    for arg in args:
        ...
        temp_df = temp_df.apply(lambda x: x.astype(str) + ' (' + (x/sum(x)).apply(lambda y: '{:.1%}'.format(y)) + ')' if x.sum() != 0 else x.astype(str) + ' (0.0%)')
...
def get_stats_perc_df(column,*dfs):
    ...
    for arg in args:
        temp_df = arg.groupby('Race').agg({'Subject': 'count'}).reindex(race_order, fill_value=0)
        # combine the percentage and count column into 1 column for variables different than 0
        temp_df = temp_df.apply(lambda x: x.astype(str) + ' (' + (x/sum(x)).apply(lambda y: '{:.1%}'.format(y)) + ')' if x.sum() != 0 else x.astype(str))    
        # merge the temp_df to the main df
        main_df = pd.concat([main_df, temp_df], axis=1)
    return main_df
