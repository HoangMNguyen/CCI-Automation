...
def DSMB12423(
    ...
    output_file_name=datetime.now().strftime("%Y%m%d%H%M%S") + "-12423-DSMB Report",
    debug=False,
):
    ...
    if not data["DM"].empty:
        ...
        enrollment_df = add_rename_column_corelisting(
            ...
            data,
            "NHLMHDIAG",
            "Primary Diagnosis of NHL (IG_NS_NA_NHLMHDIAG1.CL_NS_YH_NHLDIAG_cl_NS_NHLDIAG1)",
            "Disease NHL",
        )
        enrollment_df = add_rename_column_corelisting(
            enrollment_df,
            data,
            "NHLMHDIAG",
            "Specify Other Diagnosis (IG_NS_NA_NHLMHDIAG1.TX_NS_NH_NHLDIAGOTH)",
            "Disease NHL2",
        )
        ...
        enrollment_df["Disease Type"] = (
            enrollment_df["Disease NHL"].fillna("")
            + " "
            + enrollment_df["Disease NHL2"].fillna("")
        )
        ...
        enrollment_df["SF4"] = enrollment_df[enrollment_df["SF4"].notna()][
            "SF4"
        ].astype(str)
        enrollment_df.loc[
        ...
        index_reference = enrollment_df.columns.get_loc(
            "Subject meets all study eligibility?IE"
        )
        ...
        # Infused
        enrollment_df = add_rename_column_corelisting(
            enrollment_df, data, "EXINF", "Event Group Label", "Event Group Label"
        )
        enrollment_df = add_rename_column_corelisting(
            enrollment_df,
            data,
            "EXINF",
            "Was infusion administered? (IG_NS_NA_EXINF1.CL_NS_NH_INFADMIN_cl_YS_YN1)",
            "Infused",
        )
        ...
        grouped_df.loc[
            (
                grouped_df["Event Group Label"]
                == "Retreatment Lymphodepleting Chemotherapy"
            ),
            "Event Group Label",
        ] = "Day 0-R"
        ...
        infusion_df = add_rename_column_corelisting(
            infusion_df, data, "EXINF", "Event Group Label", "Event Group Label"
        )
        ...
        infusion_df["Date of TmCD19-IL18 Infusion"] = infusion_df[
            "Date of TmCD19-IL18 Infusion"
        ].apply(
        ...
        infusion_df["Target Cell Dose"] = infusion_df["Dose Level Assignment"].map(
            TCD_dict
        )
        ...
        infusion_df["Met Target Dose"] = infusion_df.apply(
            ...
            and row["Total TmCD19-IL18 CAR T Cell Dose Administered"]
            >= row["Target Cell Dose"]
            else "",
            axis=1,
        )
        infusion_df["Met Target Dose"] = infusion_df.apply(
        ...
        infusionR_df = add_rename_column_corelisting(
            infusionR_df, data, "EXINF", "Event Group Label", "Event Group Label"
        )
        ...
        # convert the date to datetime object and format it to MM-DD-YYYY
        infusionR_df["Date of TmCD19-IL18 Retreatment Infusion"] = infusionR_df[
            "Date of TmCD19-IL18 Retreatment Infusion"
        ].apply(
            lambda x: datetime.strptime(x, "%Y-%m-%d").strftime("%m-%d-%Y")
            if pd.notna(x)
        ...
        infusionR_df["Total Cell Dose Administered"] = infusionR_df[
            "Total Cell Dose Administered"
        ].multiply(
            10
            ** infusionR_df[
        ...
        # fillter out the rows that have NaN in Met Target %scFv
        infusionR_df["Met Target %scFv"] = infusionR_df[
            infusionR_df["Met Target %scFv"].notna()
        ]["Met Target %scFv"].apply(lambda x: "Y" if x >= 2 else "N")
        # fill NaN with empty string
        pd.set_option("future.no_silent_downcasting", True)
        ...
        infusionA_df = infusion_df[
            infusion_df["Cohort Assignment"] == "Cohort A: Non-Hodgkin Lymphoma"
        ]
        infusion_statA1 = get_stats_df(
            "Total TmCD19-IL18 CAR T Cell Dose Administered", infusionA_df
        )
        ...
        met_target_count = infusionA_df[
            infusionA_df["Met Target %scFv"] == "Y"
        ].count()["Subject"]
        ...
        DR_NHL_PET_dict = {
            ...
            #    "Indeterminate Response (IR)": 4,  # hidden during amendment
            "Progressive Metabolic Disease (PMD)": 4,  # 5
            "Not Assessed": 5,  # 6,  coded as 97 in corelisting, but 6 is used for best response score
            "Not Reported": 10,  # not in codelist cl_NS_RSNHLPET1, used to replace not assessed
        }
        ...
        DR_NHL_CT_dict = {
            ...
            "Stable Disease (SD)": 3,
            #    "Indeterminate Response (IR)": 4,  # hidden during amendment
            "Progressive Disease (PD)": 4,  # 5
            "Not Assessed": 5,  # 6, coded as 97 in corelisting, but 6 is used for best response score
            "Not Reported": 10,  # not in codelist cl_NS_RSNHLCT1, used to replace not assessed
        }
        ...
        event_A_dict = {
            ...
            "Retreatment Lymphodepleting Chemotherapy": "Pre-Retreatment",
        }
        ...
        # Get data from Initiation of Long Term Follow up from retx
        ...
        # Get data from DSINITRT, did not include subjects entering retx from LTFU
        ...
        responseA_primary_df["Event Date"] = pd.to_datetime(
            responseA_primary_df["Event Date"]
        )
        # Snapshot the responseA_primary_df
        responseA_primary_df_snapshot = responseA_primary_df.copy()

        # check the number of subject for cohort A-NHL
        ...
        if subject_A_prim_count > 0:
            responseA_primary_df.fillna(
                {
                    "PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1)": "Not Reported"
                },
                inplace=True,
            )
            ...
            responseA_primary_df.fillna(
                {
                    "CT-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLCT_cl_NS_RSNHLCT1)": "Not Reported"
                },
                inplace=True,
            )
            ...
            unique_subjects = pd.DataFrame(
                responseA_primary_df["Subject"].unique(), columns=["Subject"]
            )
            ...
            responseA_primary_current_CT_df.rename(
                columns={
            ...
            responseA_best_PET_idx = responseA_primary_df.groupby("Subject")[
                "PET-Score"
            ].idxmin()
            # Select these rows for the best PET-based response
            responseA_best_PET_df = responseA_primary_df.loc[
                responseA_best_PET_idx
            ].copy()
            ...
            # Rename the column PET-Based NHL Disease Response to PET-Based NHL Response
            responseA_best_PET_df.rename(
                columns={
                    "PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1)": "PET-Based NHL Response",
            ...
            responseA_best_CT_idx = responseA_primary_df.groupby("Subject")[
                "CT-Score"
            ].idxmin()
            ...
            # Rename the column CT-Based NHL Disease Response to CT-Based NHL Response
            responseA_best_CT_df.rename(
                columns={
                    "CT-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLCT_cl_NS_RSNHLCT1)": "CT-Based NHL Response",
            ...
            responseA_primary_D28_df = pd.concat(
                [
                    ...
                    responseA_df[
                        ~responseA_df["Subject"].isin(
                            responseA_primary_D28_df["Subject"]
                        )
                    ][["Subject"]],
                ]
            )
            # Remove duplicates
            responseA_primary_D28_df = responseA_primary_D28_df.drop_duplicates(
                subset=["Subject"]
            )
            ...
            for index, row in responseA_primary_D28_df.iterrows():
                ...
                if pd.isna(
                    ...
                    if row["Subject"] in PD_df["Subject"].values:
                        ...
                        filtered_df = temp_df[
                            (temp_df["Subject"] == row["Subject"])
                            & (temp_df["Event Date"] <= end_date)
                        ]
                    ...
                    # check if the subject is in Initiation of REtx before Day 28
                        ...
                        end_date = DSINITRT_df[
                            DSINITRT_df["Subject"] == row["Subject"]
                        ][
                            "End of Primary Follow-Up Date (IG_NS_NA_DSINITRT1.DT_NS_YH_PFUENDDAT)"
                        ].values[0]
                        # find the response of the same subject with the latest event date that is before the Day 28 event date
            ...
            # Rename the column PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1) to PET-Based NHL ORR
            responseA_primary_D28_df.rename(
                columns={
                    "PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1)": "PET-Based NHL ORR",
                    "CT-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLCT_cl_NS_RSNHLCT1)": "CT-Based NHL ORR",
                },
                inplace=True,
            )
            ...
            # Fill NaN with "Not Reported" in column PET-Based NHL ORR
            final_responseA_primary_df.fillna(
                {"PET-Based NHL ORR": "Not Reported"}, inplace=True
            )
            # Fill NaN with "Not Reported" in column CT-Based NHL ORR
            final_responseA_primary_df.fillna(
                {"CT-Based NHL ORR": "Not Reported"}, inplace=True
            )
            ...
            responseA_primary_AE_df = responseA_primary_AE_df[
                responseA_primary_AE_df[
                    "Event Onset (IG_NS_NA_AE1.CL_NS_YH_AEONSET_cl_NS_AEONSET1)"
                ]
                == "After TmCD19-IL18 CAR T Cell Administration"
            ]
            ...
            final_responseA_primary_df["AE"] = final_responseA_primary_df[
                "Subject"
            ].apply(
            ...
            final_responseA_primary_df["SAE"] = final_responseA_primary_df[
                "Subject"
            ].apply(
                lambda x: "Y"
                if x
            ...
            responseA_primary_SV_df = data["DSSV"][
                ["Subject", "Event Label", "Event Date"]
            ]
            # Getting Study Status dataframe from DSSVLTFU, column Subject, Event Label and Event Date
            responseA_primary_DSSVLTFU_df = data["DSSVLTFU"][
                ["Subject", "Event Label", "Event Date"]
            ]
            # Combine DSSVLTFU with SV dataframe vertically
            responseA_primary_SV_df = pd.concat(
                [responseA_primary_SV_df, responseA_primary_DSSVLTFU_df]
            )
            # Sort the dataframe by Subject and Event Date
            responseA_primary_SV_df = responseA_primary_SV_df.sort_values(
                by=["Subject", "Event Date"]
            )
            ...
            final_responseA_primary_df["Event Label"] = final_responseA_primary_df[
                "Event Label"
            ].map(event_A_dict)
            ...
            final_responseA_primary_df = final_responseA_primary_df[
                [
                    ...
                    "CT-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLCT_cl_NS_RSNHLCT1)",
                    "CT Current Time Point",
                    "PET-Based NHL Response",
                    "Best PET Time Point",
                    "CT-Based NHL Response",
                    "Best CT Time Point",
                    "PET-Based NHL ORR",
                    "CT-Based NHL ORR",
                    "AE",
            ...
            final_responseA_primary_df = final_responseA_primary_df.replace(
            ...
            # Convert Event Date to datetime object
            responseA_retreatment_df["Event Date"] = pd.to_datetime(
                responseA_retreatment_df["Event Date"]
            )
            ...
            if subject_A_retx_count > 0:
                ...
                responseA_retreatment_df.fillna(
                    {
                        "PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1)": "Not Reported"
                    },
                    inplace=True,
                ...
                # Get the indices of the rows with the maximum 'Event Date' for each 'Subject'
                PET_idx = responseA_retreatment_PET_df.groupby("Subject")[
                    "Event Date"
                ].idxmax()
                # Select these rows for the current response
                responseA_retreatment_current_PET_df = responseA_retreatment_PET_df.loc[
                    PET_idx
                ].copy()
                # Rename the column 'Retreatment Time Point (IG_NS_NA_NHLRS1.CL_NS_NH_RSTPTR_cl_NS_RSTPT2)' to 'PET Current Time Point'
                responseA_retreatment_current_PET_df.rename(
                    columns={
                ...
                # Get the indices of the rows with the maximum 'Event Date' for each 'Subject'
                CT_idx = responseA_retreatment_CT_df.groupby("Subject")[
                    "Event Date"
                ].idxmax()
                # Select these rows for the current response
                responseA_retreatment_current_CT_df = responseA_retreatment_CT_df.loc[
                    CT_idx
                ].copy()
                # Rename the column 'Retreatment Time Point (IG_NS_NA_NHLRS1.CL_NS_NH_RSTPTR_cl_NS_RSTPT2)' to 'CT Current Time Point'
                responseA_retreatment_current_CT_df.rename(
                    columns={
                ...
                ## Best PET-Based NHL Disease Response primary
                # Get the indices of the rows with the minimum 'PET-Best' for each 'Subject'
                responseA_best_PET_idx = responseA_retreatment_df.groupby("Subject")[
                    "PET-Score"
                ].idxmin()
                # Select these rows for the best PET-based response
                responseA_best_PET_df = responseA_retreatment_df.loc[
                    responseA_best_PET_idx
                ].copy()
                # Select the columns subject and PET-Based NHL Disease Response from responseA_best_PET_df
                responseA_best_PET_df = responseA_best_PET_df[
                ...
                # Fill NaN with "Not Reported" in column PET-Based NHL Response
                responseA_best_PET_df.fillna(
                    {"PET-Based NHL Response": "Not Reported"}, inplace=True
                )
                # Merge left with the primary current response dataframe
                final_responseA_retreatment_df = pd.merge(
                ...
                ## Best CT-Based NHL Disease Response primary
                # Get the indices of the rows with the minimum 'CT-Best' for each 'Subject'
                responseA_best_CT_idx = responseA_retreatment_df.groupby("Subject")[
                    "CT-Score"
                ].idxmin()
                # Select these rows for the best CT-based response
                responseA_best_CT_df = responseA_retreatment_df.loc[
                    responseA_best_CT_idx
                ]
                # Select the columns subject and CT-Based NHL Disease Response from responseA_best_CT_df
                responseA_best_CT_df = responseA_best_CT_df[
                    [
                ...
                # Fill NaN with "Not Reported" in column CT-Based NHL Response
                responseA_best_CT_df.fillna(
                    {"CT-Based NHL Response": "Not Reported"}, inplace=True
                )
                # Merge left with the primary response dataframe
                final_responseA_retreatment_df = pd.merge(
                    final_responseA_retreatment_df,
                ...
                responseA_retreatment_D28_df = pd.concat(
                    [
                        ...
                        responseA_df[
                            ~responseA_df["Subject"].isin(
                                responseA_retreatment_D28_df["Subject"]
                            )
                        ][["Subject"]],
                    ]
                )
                # Remove duplicates
                responseA_retreatment_D28_df = (
                    responseA_retreatment_D28_df.drop_duplicates(subset=["Subject"])
                )
                ...
                for index, row in responseA_retreatment_D28_df.iterrows():
                    ...
                    if pd.isna(
                        ...
                        if row["Subject"] in PD_Retx_df["Subject"].values:
                            ...
                            end_date = PD_Retx_df[
                                PD_Retx_df["Subject"] == row["Subject"]
                            ][
                ...
                # Rename the column PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1) to PET-Based NHL ORR
                responseA_retreatment_D28_df.rename(
                    columns={
                        "PET-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLPET_cl_NS_RSNHLPET1)": "PET-Based NHL ORR",
                        "CT-Based NHL Disease Response (IG_NS_NA_NHLRS2.CL_NS_NH_NHLCT_cl_NS_RSNHLCT1)": "CT-Based NHL ORR",
                    },
                ...
                # Fill NaN with "Not Reported" in column PET-Based NHL ORR
                final_responseA_retreatment_df.fillna(
                    {"PET-Based NHL ORR": "Not Reported"}, inplace=True
                )
                # Fill NaN with "Not Reported" in column CT-Based NHL ORR
                final_responseA_retreatment_df.fillna(
                ...
                responseA_retreatment_AE_df = responseA_retreatment_AE_df[
                    (
                        responseA_retreatment_AE_df[
                            "Event Onset (IG_NS_NA_AE1.CL_NS_YH_AEONSET_cl_NS_AEONSET1)"
                        ]
                        == "After Retreatment Lymphodepleting Chemotherapy"
                    )
                    | (
                        responseA_retreatment_AE_df[
                            "Event Onset (IG_NS_NA_AE1.CL_NS_YH_AEONSET_cl_NS_AEONSET1)"
                        ]
                        == "After Retreatment TmCD19-IL18 CAR T Cell Administration"
                    )
                ]
                ...
                final_responseA_retreatment_df["AE"] = final_responseA_retreatment_df[
                    "Subject"
                ].apply(
                    lambda x: "Y"
                    if x in responseA_retreatment_AE_df["Subject"].values
                    else "N"
                )
                ...
                final_responseA_retreatment_df["SAE"] = final_responseA_retreatment_df[
                    "Subject"
                ].apply(
                    lambda x: "Y"
                    if x
                ...
                ## Checking Study Status for NHL primary
                # Getting Study Status dataframe from SV, column Subject, Event Label and Event Date
                responseA_retreatment_SV_df = data["DSSV"][
                    ["Subject", "Event Label", "Event Date"]
                ]
                # Getting Study Status dataframe from DSSVLTFU, column Subject, Event Label and Event Date
                responseA_retreatment_DSSVLTFU_df = data["DSSVLTFU"][
                    ["Subject", "Event Label", "Event Date"]
                ]
                # Combine DSSVLTFU with SV dataframe vertically
                ...
                # Sort the dataframe by Subject and Event Date
                responseA_retreatment_SV_df = responseA_retreatment_SV_df.sort_values(
                    by=["Subject", "Event Date"]
                )
                # For each unique subject, get the last row of the dataframe
                responseA_retreatment_SV_df = responseA_retreatment_SV_df.groupby(
                    "Subject"
                ).tail(1)
                # Merge left with the current response dataframe
                final_responseA_retreatment_df = pd.merge(
                ...
                final_responseA_retreatment_df["Event Label"] = (
                    final_responseA_retreatment_df["Event Label"].map(event_A_dict)
                )
                ...
                final_responseA_retreatment_df = final_responseA_retreatment_df.replace(
                    [np.nan, np.inf, -np.inf], ""
                )
                if debug:
        ...
        AE_df = AE_df[
            (
                AE_df["Event Onset (IG_NS_NA_AE1.CL_NS_YH_AEONSET_cl_NS_AEONSET1)"]
                == "After TmCD19-IL18 CAR T Cell Administration"
            )
            | (
                AE_df["Event Onset (IG_NS_NA_AE1.CL_NS_YH_AEONSET_cl_NS_AEONSET1)"]
        ...
        total_infused_df["SAE"] = total_infused_df["Subject"].apply(
            ...
            if x
            in AE_df[
                AE_df["AE or SAE? (IG_NS_NA_AE2.CL_YS_YH_AESEV_cl_NS_AESAE1)"] == "SAE"
            ]["Subject"].values
            else "N"
        )
        ...
        if subject_A_prim_count > 0:
            ...
            response_stat_A_BOR_PET = get_stats_percentage(
                "PET-Based NHL Response", responseA_stat
            )
            response_stat_A_BOR_CT = get_stats_percentage(
                "CT-Based NHL Response", responseA_stat
            )
            response_stat_A_ORR_PET = get_stats_percentage(
                "PET-Based NHL ORR", responseA_stat
            )
            response_stat_A_ORR_CT = get_stats_percentage(
                "CT-Based NHL ORR", responseA_stat
            )
        ...
        if subject_A_prim_count > 0:
            final_responseA_primary_df.loc[
                (
                    final_responseA_primary_df["Event Label"]
                    == "Retreatment Long Term Follow-up"
                )
        ...
        with pd.ExcelWriter(
            ...
            if data["DM"]["Subject"].count() > 0:
                ...
                if enrollment_df["Subject"].count() > 0:
                    ...
                    worksheet1.merge_range(
                        "F1:I1", "Cohort A (NHL) Enrollment", bold_12_format
                    )
                    ...
                    worksheet3.merge_range(
                        "B1:D1", "Cells Infused", bold_12_wrap_format
                    )
                    worksheet3.merge_range(
                    ...
                    worksheet3.merge_range(
                        "A3:F3",
                        "Cohort A (N=" + str(infusion_count[0]) + ")",
                        bold_12_wrap_format,
                    )
                    ...
                    worksheet3.merge_range(
                        "F4:F6", infusion_statA.iloc[0, 4], normal_data_format
                    )
                    ...
                    worksheet4.merge_range(
                        "F1:F2", "Date of TmCD19-IL18 Infusion", bold_12_wrap_format
                    )
                    worksheet4.merge_range(
                        "G1:J1", "Cells Infused", bold_12_wrap_format
                    )
                    worksheet4.merge_range(
                        "K1:L1", "Transduction Efficiency", bold_12_wrap_format
                    )
                    ...
                    worksheet4.write(
                        "I2", "Total Cell Dose Administered", bold_12_wrap_format
                    )
                    ...
                    worksheet4.merge_range(
                        "P1:P2", "Study Day (Retreatment)", bold_12_wrap_format
                    )
                    worksheet4.merge_range(
                        "Q1:Q2", "Cohort Assignment", bold_12_wrap_format
                    )
                    worksheet4.merge_range(
                        "R1:R2",
                        "Lymphodepleting Chemotherapy Regimen",
                    ...
                    worksheet4.merge_range(
                        "T1:U1", "Cells Infused", bold_12_wrap_format
                    )
                    worksheet4.merge_range(
                        "V1:W1", "Transduction Efficiency", bold_12_wrap_format
                    )
                    worksheet4.write(
                        "T2",
                        "Total TmCD19-IL18 CAR T Cell Dose Administered",
                    ...
                    worksheet4.write(
                        "U2", "Total Cell Dose Administered", bold_12_wrap_format
                    )
                        ...
                        for i in range(0, len(response_stat_A_BOR_PET)):
                            for j in range(0, len(response_stat_A_BOR_PET.columns)):
                                worksheet5.write(
                                    i + 3,
                                    j + 1,
                                    response_stat_A_BOR_PET.iloc[i, j],
                                    normal_data_format,
                                )
                        for i in range(0, len(response_stat_A_BOR_CT)):
                            for j in range(0, len(response_stat_A_BOR_CT.columns)):
                                worksheet5.write(
                                    i + 3,
                                    j + 3,
                                    response_stat_A_BOR_CT.iloc[i, j],
                                    normal_data_format,
                                )
                        for i in range(0, len(response_stat_A_ORR_PET)):
                            for j in range(0, len(response_stat_A_ORR_PET.columns)):
                                worksheet5.write(
                                    ...
                                    i + 9,
                                    j + 1,
                                    response_stat_A_ORR_PET.iloc[i, j],
                                    normal_data_format,
                                )
                        for i in range(0, len(response_stat_A_ORR_CT)):
                            for j in range(0, len(response_stat_A_ORR_CT.columns)):
                                worksheet5.write(
                                    ...
                                    i + 9,
                                    j + 3,
                                    response_stat_A_ORR_CT.iloc[i, j],
                                    normal_data_format,
                                )
                    ...
                    worksheet5.merge_range(
                        "I2:J2", "Serious Adverse Events ", bold_11_format
                    )
                    ...
                    worksheet5.write(
                        "F4", "All Cohorts", bold_11_format
                    )  # Currently only has Cohort A, will use filter for each cohort when there are more cohorts

                    # Response Headers
                    worksheet5.merge_range(
                        "A1:D1",
                        "Cohort A (NHL) Subject Response (N="
                        + str(subject_A_prim_count)
                        + ")",
                        bold_12_format,
                    )
                    worksheet5.merge_range(
                        "A2:B2", "PET-Based NHL Response", bold_11_format
                    )
                    worksheet5.merge_range(
                        "C2:D2", "CT-Based NHL Response", bold_11_format
                    )
                    worksheet5.merge_range(
                        "A3:D3", "Best Overall Response (BOR)", bold_11_format
                    )
                    worksheet5.merge_range(
                        "A9:D9",
                    ...
                    if subject_A_prim_count > 0:
                        ...
                        if subject_A_prim_count > 0:
                            for i in range(0, len(final_responseA_primary_df)):
                                for j in range(
                                    0, len(final_responseA_primary_df.columns)
                                ):
                                    worksheet6.write(
                                        i + 3,
                                        j,
                                        final_responseA_primary_df.iloc[i, j],
                                        normal_data_format,
                                    )
                            if subject_A_retx_count > 0:
                                for i in range(0, len(final_responseA_retreatment_df)):
                                    for j in range(
                                        0, len(final_responseA_retreatment_df.columns)
                                    ):
                                        worksheet6.write(
                                            i + 3,
                                            j + 16,
                                            final_responseA_retreatment_df.iloc[i, j],
                                            normal_data_format,
                                        )
                        # * WRITING HEADER AND FORMATTING
                        if subject_A_prim_count > 0:
                            ...
                            worksheet6.merge_range(
                                "A2:A3", "Subject ID", bold_11_format
                            )
                            worksheet6.merge_range(
                                "B2:E2", "Current Response", bold_11_format
                            )
                            worksheet6.merge_range(
                                "F2:I2", "Best Response/Timepoint", bold_11_format
                            )
                            worksheet6.merge_range(
                                "J2:K2", "Overall Response/Day 28", bold_11_format
                            )
                            worksheet6.write(
                                "B3", "PET-Based NHL Response", bold_11_format
                            )
                            worksheet6.write("C3", "Study Timepoint", bold_11_format)
                            worksheet6.write(
                                "D3", "CT-Based NHL Response", bold_11_format
                            )
                            worksheet6.write("E3", "Study Timepoint", bold_11_format)
                            worksheet6.write(
                                "F3", "PET-Based NHL Response", bold_11_format
                            )
                            worksheet6.write("G3", "Study Timepoint", bold_11_format)
                            worksheet6.write(
                                "H3", "CT-Based NHL Response", bold_11_format
                            )
                            worksheet6.write("I3", "Study Timepoint", bold_11_format)
                            worksheet6.write("J3", "PET-Based NHL ORR", bold_11_format)
                            worksheet6.write("K3", "CT-Based NHL ORR", bold_11_format)
                            worksheet6.merge_range(
                                "L2:L3", "Adverse Events \n(Y/N)", bold_11_wrap_format
                            )
                            worksheet6.merge_range(
                                "M2:M3",
                            ...
                            worksheet6.merge_range(
                                "N2:N3", "Study Status", bold_11_wrap_format
                            )
                            if subject_A_retx_count > 0:
                                worksheet6.merge_range(
                                    "Q1:AD1",
                                ...
                                worksheet6.merge_range(
                                    "Q2:Q3", "Subject ID", bold_11_format
                                )
                                worksheet6.merge_range(
                                    "R2:U2", "Current Response", bold_11_format
                                )
                                worksheet6.merge_range(
                                    "V2:Y2", "Best Response/Timepoint", bold_11_format
                                )
                                worksheet6.merge_range(
                                    "Z2:AA2", "Overall Response/Day 28", bold_11_format
                                )
                                worksheet6.write(
                                    "R3", "PET-Based NHL Response", bold_11_format
                                )
                                worksheet6.write(
                                    "S3", "Study Timepoint", bold_11_format
                                )
                                worksheet6.write(
                                    "T3", "CT-Based NHL Response", bold_11_format
                                )
                                worksheet6.write(
                                    "U3", "Study Timepoint", bold_11_format
                                )
                                worksheet6.write(
                                    "V3", "PET-Based NHL Response", bold_11_format
                                )
                                worksheet6.write(
                                    "W3", "Study Timepoint", bold_11_format
                                )
                                worksheet6.write(
                                    "X3", "CT-Based NHL Response", bold_11_format
                                )
                                worksheet6.write(
                                    "Y3", "Study Timepoint", bold_11_format
                                )
                                worksheet6.write(
                                    "Z3", "PET-Based NHL ORR", bold_11_format
                                )
                                worksheet6.write(
                                    "AA3", "CT-Based NHL ORR", bold_11_format
                                )
                                worksheet6.merge_range(
                                    "AB2:AB3",
                                    "Adverse Events \n(Y/N)",
                                ...
                                worksheet6.merge_range(
                                    "AD2:AD3", "Study Status", bold_11_wrap_format
                                )
                        # Autofit
                        worksheet6.autofit()
